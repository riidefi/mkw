...
GHTTPRequest ghttpGetExA(const char* URL, const char* headers, char* buffer,
                         int bufferSize, GHTTPPost post, GHTTPBool throttle,
                         GHTTPBool blocking,
                         ghttpProgressCallback progressCallback,
                         ghttpCompletedCallback completedCallback, void* param);
GHTTPRequest ghttpSaveExA(const char* URL, const char* filename,
                          const char* headers, GHTTPPost post,
                          GHTTPBool throttle, GHTTPBool blocking,
                          ghttpProgressCallback progressCallback,
                          ghttpCompletedCallback completedCallback,
                          void* param);
GHTTPRequest ghttpStreamExA(const char* URL, const char* headers,
                            GHTTPPost post, GHTTPBool throttle,
                            GHTTPBool blocking,
                            ghttpProgressCallback progressCallback,
                            ghttpCompletedCallback completedCallback,
                            void* param);
GHTTPRequest ghttpHeadExA(const char* URL, const char* headers,
                          ...
                          ghttpCompletedCallback completedCallback,
                          void* param);
GHTTPRequest ghttpPostExA(const char* URL, const char* headers, GHTTPPost post,
                          GHTTPBool throttle, GHTTPBool blocking,
                          ghttpProgressCallback progressCallback,
                          ghttpCompletedCallback completedCallback,
                          void* param);
...
#endif
    // Log buffer data
    ghiLogResponse(connection->getFileBuffer.data,
                   connection->getFileBuffer.len);
...
GHTTPRequest ghttpGetA(const char* URL, GHTTPBool blocking,
                       ghttpCompletedCallback completedCallback, void* param) {
  return ghttpGetExA(URL, NULL, NULL, 0, NULL, GHTTPFalse, blocking, NULL,
...
GHTTPRequest ghttpGetExA(const char* URL, const char* headers, char* buffer,
  ...
  if (connection->userBufferSupplied)
    bResult = ghiInitFixedBuffer(connection, &connection->getFileBuffer, buffer,
                                 bufferSize);
  else
    bResult = ghiInitBuffer(connection, &connection->getFileBuffer,
                            GET_FILE_BUFFER_INITIAL_SIZE,
...
GHTTPRequest ghttpSaveA(const char* URL, const char* filename,
                        GHTTPBool blocking,
                        ghttpCompletedCallback completedCallback, void* param) {
  return ghttpSaveExA(URL, filename, NULL, NULL, GHTTPFalse, blocking, NULL,
                      completedCallback, param);
}

static GHTTPRequest _ghttpSaveEx(const char* URL, const gsi_char* filename,
                                 const char* headers, GHTTPPost post,
                                 GHTTPBool throttle, GHTTPBool blocking,
                                 ghttpProgressCallback progressCallback,
                                 ghttpCompletedCallback completedCallback,
...
GHTTPRequest ghttpSaveExA(const char* URL, const char* filename,
                          ...
                          ghttpCompletedCallback completedCallback,
                          void* param) {
  return _ghttpSaveEx(URL, filename, headers, post, throttle, blocking,
                      progressCallback, completedCallback, param);
}

GHTTPRequest ghttpStreamA(const char* URL, GHTTPBool blocking,
                          ghttpProgressCallback progressCallback,
                          ghttpCompletedCallback completedCallback,
                          void* param) {
  return ghttpStreamExA(URL, NULL, NULL, GHTTPFalse, blocking, progressCallback,
                        completedCallback, param);
}
...
GHTTPRequest ghttpHeadA(const char* URL, GHTTPBool blocking,
                        ghttpCompletedCallback completedCallback, void* param) {
  return ghttpHeadExA(URL, NULL, GHTTPFalse, blocking, NULL, completedCallback,
                      param);
}
...
GHTTPRequest ghttpPostA(const char* URL, GHTTPPost post, GHTTPBool blocking,
                        ghttpCompletedCallback completedCallback, void* param) {
  return ghttpPostExA(URL, NULL, post, GHTTPFalse, blocking, NULL,
                      completedCallback, param);
}
...
void ghttpSetThrottle(GHTTPRequest request, GHTTPBool throttle) {
  ...
  if (connection->socket != INVALID_SOCKET)
    SetReceiveBufferSize(connection->socket,
                         throttle ? ghiThrottleBufferSize : (8 * 1024));
}
...
// Call this in the completed callback to obtain the socket, which can be used
// with ghttpReuseSocket to make a second request to the same host
///////////////////////////////////////////////////////////////////
SOCKET ghttpGetSocket(GHTTPRequest request) {
  GHIConnection* connection;
  SOCKET ret;
  ...
  // Only allow them to grab the socket during the competion callback (not while
...
// For use in persistent HTTP connections
// Call this after creating a request, but before calling think the first time
// in order to reuse an existing connection to the same server If the socket
// passed is INVALID_SOCKET, a new connection will be created and marked as
// persistent
///////////////////////////////////////////////////////////////////
GHTTPBool ghttpReuseSocket(GHTTPRequest request, SOCKET socket) {
  GHIConnection* connection;
  ...
  {
          // The connection is marked as persistent, but we still need to lookup
  and connect, so don't advance the state
          /////////////////////////////////////////////////////////////////////
          return GHTTPTrue;
  }
...
GHTTPBool ghttpPostAddStringA(GHTTPPost post, const char* name,
                              const char* string) {
  // assert(post);
  // assert(name && name[0]);
...
GHTTPBool ghttpPostAddFileFromDiskA(GHTTPPost post, const char* name,
                                    const char* filename,
  ...
  return ghiPostAddFileFromDisk(post, name, filename, reportFilename,
                                contentType);
}

GHTTPBool ghttpPostAddFileFromMemoryA(GHTTPPost post, const char* name,
  ...
  return ghiPostAddFileFromMemory(post, name, buffer, bufferLen, reportFilename,
                                  contentType);
}
...
void ghttpPostSetCallback(GHTTPPost post, ghttpPostCallback callback,
                          void* param) {
  // assert(post);
