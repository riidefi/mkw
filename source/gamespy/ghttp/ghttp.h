...
typedef enum {
  ...
  GHTTPBufferOverflow, // 2:  The user-supplied buffer was too small to hold the
                       // file.
  ...
  GHTTPFileWriteFailed,  // 13: An error occured writing to the local file (for
                         // ghttpSaveFile[Ex]).
  GHTTPFileReadFailed, // 14: There was an error reading from a local file (for
                       // posting files from disk).
  GHTTPFileIncomplete, // 15: Download started but was interrupted.  Only
                       // reported if file size is known.
  GHTTPFileToBig, // 16: The file is to big to be downloaded (size exceeds range
                  // of interal data types)
...
typedef enum {
  ...
  GHTTPEncryptionEngine_GameSpy, // must add /common/gsSSL.h and /common/gsSSL.c
                                 // to project
  GHTTPEncryptionEngine_MatrixSsl, // must define MATRIXSSL and include
                                   // matrixssl source files
  GHTTPEncryptionEngine_RevoEx, // must define REVOEXSSL and include RevoEX SSL
                                // source files

  GHTTPEncryptionEngine_Default // Will use GameSpy unless another engine is
                                // defined
...
#else
// Backwards compatibility, developers may have relied on -1 as the only error
// code
typedef enum {
  GHTTPErrorStart = -1,
...
typedef void (*ghttpProgressCallback)(
    ...
    const char*
        buffer, // The file's bytes so far, NULL if state<GHTTPReceivingFile.
    GHTTPByteCount bufferLen,     // The number of bytes in the buffer, 0 if
                                  // state<GHTTPReceivingFile.
    GHTTPByteCount bytesReceived, // The total number of bytes receivied, 0 if
                                  // state<GHTTPReceivingFile.
...
//////////////////////////////////
GHTTPRequest
ghttpGet(const gsi_char* URL, // The URL for the file
                              // ("http://host.domain[:port]/path/filename").
         GHTTPBool blocking, // If true, this call doesn't return until the file
                             // has been recevied.
         ghttpCompletedCallback
             completedCallback, // Called when the file has been received.
...
GHTTPRequest ghttpGetEx(
    const gsi_char* URL,     // The URL for the file
                             // ("http://host.domain[:port]/path/filename").
    const gsi_char* headers, // Optional headers to pass with the request.  Can
                             // be NULL or "".
    char* buffer,   // Optional user-supplied buffer.  Set to NULL to have one
                    // allocated.
    int bufferSize, // The size of the user-supplied buffer in bytes.  0 if
                    // buffer is NULL.
...
GHTTPRequest ghttpStream(
    ...
    GHTTPBool blocking,  // If true, this call doesn't return until the file has
                         // finished streaming.
    ghttpProgressCallback
        progressCallback, // Called whenever new data is received.
    ghttpCompletedCallback
        completedCallback, // Called when the file has finished streaming.
    void* param            // User-data to be passed to the callbacks.
);
...
GHTTPRequest ghttpHead(
    ...
    ghttpCompletedCallback
        completedCallback, // Called when the request has finished.
    void* param            // User-data to be passed to the callbacks.
);
...
// uploaded. No data will be returned from this request.  If data is needed, use
// one of the ghttp*FileEx() functions, and pass in a GHTTPPost object. Returns
// GHTTPRequestError if an error occurs.
///////////////////////////////////////////////////////////////////////////////////
...
// uploaded. No data will be returned from this request.  If data is needed, use
// one of the ghttp*FileEx() functions, and pass in a GHTTPPost object. Returns
// GHTTPRequestError if an error occurs. Allows optional extra http headers and
// an optional progress callback.
///////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////
GHTTPBool
ghttpPostAddString(GHTTPPost post,        // The post object to add to.
...
GHTTPBool ghttpPostAddFileFromDisk(
    ...
    const gsi_char*
        filename, // The name (and possibly path) to the file to upload.
...
void ghttpPostSetCallback(
    ...
    ghttpPostCallback
        callback, // The callback to call when using this post object.
...
#define ghttpGetFileEx(a, b, c, d, e, f, g, h, i, j)                           \
  ghttpGetEx(a, b, c, d, e, f, g, h, i, j)
#define ghttpSaveFile(a, b, c, d, e) ghttpSave(a, b, c, d, e)
#define ghttpSaveFileEx(a, b, c, d, e, f, g, h, i)                             \
  ghttpSaveEx(a, b, c, d, e, f, g, h, i)
#define ghttpStreamFile(a, b, c, d, e) ghttpStream(a, b, c, d, e)
#define ghttpStreamFileEx(a, b, c, d, e, f, g, h)                              \
  ghttpStreamEx(a, b, c, d, e, f, g, h)
...
GHTTPRequest
ghttpGetA(const char* URL,    // The URL for the file
                              // ("http://host.domain[:port]/path/filename").
          GHTTPBool blocking, // If true, this call doesn't return until the
                              // file has been recevied.
          ghttpCompletedCallback
              completedCallback, // Called when the file has been received.
          void* param            // User-data to be passed to the callbacks.
);
