...
void ghiDoSocketInit(GHIConnection* connection) {
  ...
  if ((connection->protocol == GHIHttps) &&
    ...
    // ghttpSetRequestEncryptionEngine(connection->request,
    // GHTTPEncryptionEngine_GameSpy);

    // 02OCT07 BED: Design changed so that only one engine can be active at a
    // time
    ...
    ghttpSetRequestEncryptionEngine(connection->request,
                                    GHTTPEncryptionEngine_Default);
  } else if ((connection->protocol != GHIHttps) &&
             (connection->encryptor.mEngine != GHTTPEncryptionEngine_None)) {
    // URL is not secured
    ghttpSetRequestEncryptionEngine(connection->request,
  ...
  if ((connection->protocol == GHIHttps) &&
    ...
    gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_State, GSIDebugLevel_Debug,
                  "Initializing SSL engine\n");
    aResult =
        (connection->encryptor.mInitFunc)(connection, &connection->encryptor);
...
void ghiDoHostLookup(GHIConnection* connection) {
  ...
  if (connection->serverIP == INADDR_NONE) {
    ...
    if (host == NULL) {
      gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_State,
                    GSIDebugLevel_HotError, "Host Lookup failed\n");
      connection->completed = GHTTPTrue;
    ...
    if (gsiStartResolvingHostname(server, &(connection->handle)) == -1) {
      gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_State,
                    GSIDebugLevel_HotError, "Thread Creation Failed\n");
...
void ghiDoConnecting(GHIConnection* connection) {
  ...
  if (connection->socket == INVALID_SOCKET) {
    ...
    int var_28 = 0x2000;
    if (setsockopt(connection->socket, 0xffff, 0x1001, (char*)(&var_28), 4) <
        0) {
      connection->completed = GHTTPTrue;
      connection->result = GHTTPSocketFailed;
      connection->socketError = GOAGetLastError(connection->socket);
...
void ghiDoSecuringSession(GHIConnection* connection) {
  // Client sends hello
  // Server sends hello, [certificate], [certificate request], [server key
  // exchange] Client sends client <key exchange>, <finished>, [certificate],
  // [certificate verify] Server sends finished
  ...
  if (connection->encryptor.mSessionStarted == GHTTPFalse) {
    ...
    if (connection->encryptor.mStartFunc != NULL) {
      aResult = (connection->encryptor.mStartFunc)(connection,
  ...
  if (connection->encryptor.mLibSendsHandshakeMessages) {
    GS_ASSERT(connection->encryptor.mStartFunc != NULL);
    if (connection->encryptor.mStartFunc != NULL) {
      GHIEncryptionResult aResult = (connection->encryptor.mStartFunc)(
          connection, &connection->encryptor);
      if (aResult == GHIEncryptionResult_Error) {
        connection->completed = GHTTPTrue;
        connection->result = GHTTPEncryptionError;
        return;
    ...
    // Continue to send and receive handshake messages until the session has
    // been secured Send any session messages
...
void ghiDoSendingRequest(GHIConnection* connection) {
  ...
  if (!connection->sendBuffer.len) {
    ...
    //    (e.g. for efficiency and testing purposes we may want to encrypt in
    //    larger blocks)
    ...
    /////////////////////////////
    if (connection->sendHeaders == NULL ||
        strstr(connection->sendHeaders, "User-Agent") == NULL)
    ...
    if (connection->post && !connection->postingState.completed) {
      ...
      ///////////////////////////////
      ghiAppendHeaderToBuffer(writeBuffer, "Content-Type",
                              ghiPostGetContentType(connection));
    }
    ...
    // Encrypt it, if necessary.  This copy is unfortunate since matrixSsl can't
    // encrypt in place
    if (connection->encryptor.mEngine != GHTTPEncryptionEngine_None &&
        connection->encryptor.mEncryptOnBuffer == GHTTPTrue) {
      GS_ASSERT(writeBuffer == &connection->encodeBuffer);
      if (!ghiEncryptDataToBuffer(&connection->sendBuffer, writeBuffer->data,
                                  writeBuffer->len)) {
        connection->completed = GHTTPTrue;
        connection->result = GHTTPEncryptionError;
        return;
...
////////////////////////
  ...
  if (connection->sendBuffer.pos != oldPos)
    ghiLogRequest(connection->sendBuffer.data + oldPos,
                  connection->sendBuffer.pos - oldPos);
#endif
...
static GHTTPBool ghiParseStatus(GHIConnection* connection) {
  ...
  // Xbox 360 needs "%n" to be manually enabled
  {
    ...
    ////////////////////
    rcode =
        sscanf(connection->recvBuffer.data, "HTTP/%d.%d %d%n", &majorVersion,
               &minorVersion, &statusCode, &statusStringIndex);
  ...
  if ((rcode != 3) || // Not all fields read.
                      //!*statusString ||  // No status string.  PANTS|9.16.02 -
                      //! apparently some servers don't return a status string
  ...
  /////////////////////////////////////////////
  while ((c = connection->recvBuffer.data[statusStringIndex]) != '\0' &&
         isspace(c))
    statusStringIndex++;
    ...
    *endOfStatus = '\0';
    ...
    if (connection->statusCode == 100 &&
      ...
      gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                    GSIDebugLevel_Comment, "Got HTTP continue\r\n");
    } else {
...
////////////////////////////////////////////////////////
static GHTTPBool ghiDeliverIncomingFileData(GHIConnection* connection,
                                            char* data, int len) {
  char* buffer = NULL;
  int bufferLen = 0;
  ...
  //////////////////////////////
  if (connection->fileBytesReceived == connection->totalSize ||
...
///////////////////////////////////////////////
static void ghiAppendToChunkHeaderBuffer(GHIConnection* connection, char* data,
                                         int len) {
  assert(connection);
  assert(data);
  assert(len >= 0);
  ...
  if (connection->chunkHeaderLen < CHUNK_HEADER_SIZE) {
    ...
    //////////////////////////////////////////////////////
    memcpy(connection->chunkHeader + connection->chunkHeaderLen, data,
...
// Returns false on error.
////////////////////////////////////////////////////////
static GHTTPBool ghiProcessIncomingFileData(GHIConnection* connection,
                                            char* data, int len) {
  assert(connection);
  assert(data);
  assert(len > 0);
  ...
  if (connection->chunkedTransfer) {
    ...
    while (len > 0) {
      ...
      if (connection->chunkReadingState == CRHeader) {
        ...
        if (endOfHeader) {
          ...
          if (connection->chunkBytesLeft == 0) {
            ...
            gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                          GSIDebugLevel_RawDump, "Reading footer\n");
          } else {
            connection->chunkReadingState = CRChunk;
            gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                          GSIDebugLevel_RawDump, "Reading %d byte chunk\n",
      ...
      ///////////////////
        ...
        gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                      GSIDebugLevel_RawDump, "Read %d bytes of chunk\n",
                      numBytes);
      ...
      /////////////////////////////////
        ...
        gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                      GSIDebugLevel_RawDump, "Read chunk footer\n");
      }
      // Reading the footer?
      //////////////////////
        ...
        gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                      GSIDebugLevel_RawDump, "Finished reading chunks\n");
...
void ghiDoReceivingHeaders(GHIConnection* connection) {
  ...
  if (((connection->statusCode / 100) == 1) &&
      (strncmp(headers, "\r\n", 2) == 0 ||
       strncmp(headers, "\xA\xA", 2) == 0)) {
    // If a continue doesn't have a header (immediate CRLF) move on to next
    // status
  ...
  if (!endOfHeaders) {
    endOfHeaders =
        strstr(headers, "\xA\xA"); // some servers seem to use LFs only?! Seen
                                   // in 302 redirect. (28may01/bgw)
  }
  if (endOfHeaders) {
    char* fileStart;
    int fileLength;
    ...
    *endOfHeaders = '\0';
    ...
    fileLength = (connection->recvBuffer.len -
                  (fileStart - connection->recvBuffer.data));

    // Set the headers buffer's new length.
    ///////////////////////////////////////
    connection->recvBuffer.len =
        (endOfHeaders - connection->recvBuffer.data + 1);
    ...
    if ((connection->statusCode / 100) == 1) {
      if (fileLength) {
        ...
        ////////////////////////////////////////////
        memmove(connection->recvBuffer.data, fileStart,
      ...
      if (location) {
        ...
        *end = '\0';
        ...
        if (*location == '/') {
          ...
          ///////////////////////////////
          len = (int)(strlen(connection->serverAddress) + 13 +
                      strlen(location) + 1);
          ...
          sprintf(connection->redirectURL, "http://%s:%d%s",
                  connection->serverAddress, connection->serverPort, location);
        } else {
    ...
    // Check the chunky.
    ////////////////////
    connection->chunkedTransfer =
        (strstr(headers, "Transfer-Encoding: chunked") != NULL) ? GHTTPTrue
                                                                : GHTTPFalse;
    if (connection->chunkedTransfer) {
      connection->chunkHeader[0] = '\0';
      connection->chunkHeaderLen = 0;
...
void ghiDoReceivingFile(GHIConnection* connection) {
  ...
  while (!connection->completed && (running_time < connection->maxRecvTime)) {
    ...
    if (result == GHIConnClosed) {
      ...
      if (connection->totalSize > 0 &&
          connection->fileBytesReceived < connection->totalSize)
    ...
    if (connection->encryptor.mEngine != GHTTPEncryptionEngine_None &&
      ...
      if (decryptedLen) {
        ...
        ////////////////////
        decryptedData =
            (connection->recvBuffer.data + connection->recvBuffer.pos);
        if (!ghiProcessIncomingFileData(connection, decryptedData,
                                        decryptedLen))
          goto _exit;
      }
    } else {
