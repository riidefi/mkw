...
typedef struct GHIConnection {
  ...
  GHTTPBool
      persistConnection; // If TRUE, Connection: close will not be sent in the
  ...
  int socketError; // If there was a socket error, the last error code is stored
                   // here.

  GHIBuffer sendBuffer;   // The buffer for outgoing data.
  GHIBuffer encodeBuffer; // The buffer for outgoing data. (will be encrypted;
                          // only used with https)
  GHIBuffer recvBuffer;   // The buffer for incoming data. (plain text)
  GHIBuffer decodeBuffer; // The buffer for incoming data. (encrypted)(only used
                          // with https)

  GHIBuffer getFileBuffer; // ghttpGetFile[Ex] uses this buffer (which may be
                           // user-supplied).
  GHTTPBool userBufferSupplied; // True if a user buffer was supplied.

  int statusMajorVersion; // The major-version number from the server's
                          // response.
  int statusMinorVersion; // The minor-version number from the server's
                          // response.
  int statusCode;         // The status-code from the server's response.
  int statusStringIndex;  // Index in the recvBuffer where the status string
                          // starts.

  int headerStringIndex; // Index in the recvBuffer where the headers begin

  GHTTPBool completed; // This connection is completed - call the callback and
                       // kill it.
  ...
  char* redirectURL; // If this is not NULL, we need to redirect the download to
                     // this URL.
  int redirectCount; // Number of redirections done for this request.

  GHTTPBool chunkedTransfer; // The body of the response is chunky
                             // ("Transfer-Encoding: chunked").
  char chunkHeader[CHUNK_HEADER_SIZE +
                   1]; // Partial chunk headers are stored in here.
  int chunkHeaderLen;  // The number of bytes in chunkHeader.
  int chunkBytesLeft;  // Number of bytes left in the chunk (only valid for
                       // CRChunk).
  CRState chunkReadingState; // Determines if a chunk header or chunk data is
                             // being read.

  GHTTPBool processing; // If true, being processed.  Used to prevent recursive
                        // processing.
  GHTTPBool connectionClosed; // If true, the connection has been closed
                              // (orderly or abortive)

  GHTTPBool throttle; // If true, throttle this connection.
  gsi_time
      lastThrottleRecv; // The last time we received on a throttled connection.

  GHTTPPost post; // If not NULL, a reference to a post object to upload with
                  // the request.
  GHIPostingState postingState; // If posting, the state of the upload.

  gsi_time maxRecvTime;      // Max time spent receiving per call to "Think" -
                             // Prevents blocking on ultrafast connections
  char* proxyOverrideServer; // Allows use of a different proxy than the global
                             // proxy
