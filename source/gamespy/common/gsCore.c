...
void gsCoreInitialize() {
  ...
  if (gsiInterlockedIncrement(&aCore->mRefCount) == 1) {
    ...
    // this is necessary in case another thread began shutdown before we
    ...
    // we may need to wait a few times if the shutdown does not complete
    // immediately
    while (gsi_is_true(aCore->mIsShuttingDown))
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void gsiCoreTaskDispatchCallback(GSTask* theTask,
                                        GSTaskResult theResult) {
...
GSTaskResult gsCoreTaskThink(GSTask* theTask) {
  ...
  if ((!theTask->mIsCanceled) && (aResult == GSTaskResult_InProgress)) {
    if ((theTask->mTimeout != 0) &&
        (current_time() - theTask->mStartTime > theTask->mTimeout)) {
      // Cancel the task...
  ...
  //       while this continue to return InProgress meaning "still needs to be
  //       pumped"
...
// cancelled tasks continue processing until the cancel is acknowledge by the
// task
#ifdef GSICORE_DYNAMIC_TASK_LIST
  {
    int len = ArrayLength(aCore->mTaskArray);
