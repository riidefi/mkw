...
// The UDP Engine should try the MSS (Maximum Segment Size = IP Packet size - IP
// Header) as a default size to keep the packets from being fragmented.
// Currently 1460 is the MSS for windows.  Consoles may have a different size.
...
typedef enum _GSUdpCloseReason {
  ...
  // errors:
  GS_UDP_CLOSED_BY_COMM_ERROR, // An invalid message was received (it was either
...
// app Request attempt callback used to tell registered listeners about
// connection attempts
typedef void (*gsUdpAppConnectAttemptCallback)(
    unsigned int theIp, unsigned short thePort, int theLatency,
    unsigned char* theInitMsg, unsigned int theInitMsgLen, void* theUserData);

// peer communication channel callback types
typedef void (*gsUdpConnClosedCallback)(unsigned int ip, unsigned short port,
                                        GSUdpCloseReason reason,
                                        void* theUserData);
typedef void (*gsUdpConnReceivedDataCallback)(
    unsigned int ip, unsigned short port, unsigned char* message,
...
typedef void (*gsUdpConnConnectedCallback)(unsigned int ip, unsigned short port,
                                           ...
                                           gsi_bool rejected,
                                           void* theUserData);
typedef void (*gsUdpConnPingCallback)(unsigned int ip, unsigned short port,
                                      unsigned int latency, void* theUserData);
...
// than 1500 bytes because of set MTU sizes. The recommended outgoing buffer
// size is the default (1460).  So take that, and subtract 16 for message
// handler header and reliable message header (if sending data reliably).
// freeSpace = 1460 - 16 - 7
...
// This function should be called for those parts of the code that want specific
// handling of messages Any call to send should include the header registered
// here.
GSUdpErrorCode
