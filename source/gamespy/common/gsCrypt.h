...
#define GS_CRYPT_RSA_DATABLOCKSIZE                                             \
  (GS_CRYPT_RSA_BYTE_SIZE - GS_CRYPT_HASHSIZE - 1)
...
//     OAEP:  62-bytes when using 1024-bit encryption
//     (GS_CRYPT_RSA_BYTE_SIZE-2*GS_CRYPT_MD5_HASHSIZE-2) PKCS1: 117-bytes when
//     using 1024-bit encryption (GS_CRYPT_RSA_BYTE_SIZE-11)
gsi_i32
gsCryptRSAEncryptBuffer(const gsCryptRSAKey* publicKey,
...
// These require the private key, which only the server should have. Included
// here for test purposes
gsi_i32
gsCryptRSADecryptBuffer(const gsCryptRSAKey* privateKey,
                        const unsigned char cipherText[GS_CRYPT_RSA_BYTE_SIZE],
                        unsigned char* plainTextOut, gsi_u32* lenOut);
//     Note: There is a debate on whether or not to sign-first-then-encrypt, or
//     encrypt-first-then-sign
//           SignFirst: Decryption must take place before the signature is
//           validated.  This is a high overhead for invalid signatures.
//           EncryptFirst: Signature validation takes place before decryption,
//           but the MAC is unencrypted to packet sniffers.  (Exposes it to
//           attack)
//     We use SignFirst since it's unlikely a client will receive a invalid
//     signature DOS attack.
