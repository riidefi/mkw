...
#define MEM_PROFILE                                                            \
  (1) // if on additional memprofiling code will be enabled for such things as
      // high water mark calcs
...
static gsMemMgrContext MemTypeStack[MEM_CONTEXT_STACK_MAX] = {
    gsMemMgrContext_Default};
...
#define MEMALIGN_POWEROF2(x, a)                                                \
  (((gsi_uint)(x) + (a - 1)) & ~(((gsi_uint)(a)) - 1))
...
void MEM_CHUNKAlloc(MEM_CHUNK* _this, gsi_u8 _MemType, size_t _UsedSize) {
  _UsedSize = MEMALIGN_POWEROF2(
      _UsedSize, 4); // The lower 2 bits are zero, so we don't store them.
...
void MEM_CHUNKMemUsedSet(MEM_CHUNK* _this, gsi_u32 size) {
  _this->MEM_UsageStat.MemUsed = (MEMALIGN_POWEROF2(size, 4) << 6) +
                                 _this->MEM_UsageStat.MEM_TypeStruct.MemType;
}
...
MEM_CHUNK* MEM_CHUNK_POOLFindNextFreeChunk(MEM_CHUNK_POOL* _this,
                                           MEM_CHUNK* header);
void MEM_CHUNK_POOLSplitChunk(MEM_CHUNK_POOL* _this, MEM_CHUNK* header,
                              gsi_bool ReAlloc);
...
MEM_CHUNK* MEM_CHUNK_POOLAllocChunk(
    ...
    gsi_bool Backwards); // int Alignment = PTR_ALIGNMENT,
                         // gsi_bool Backwards = gsi_false);

// move a chunk within the limits of prev + prev_size and next - this_size
void MEM_CHUNK_POOLChunkMove(MEM_CHUNK_POOL* _this, MEM_CHUNK* oldpos,
                             MEM_CHUNK* newpos);

// public
/***************************************/
void MEM_CHUNK_POOLCreate(MEM_CHUNK_POOL* _this, const char* szName, char* ptr,
                          gsi_u32 _size);
...
/***************************************/
void* MEM_CHUNK_POOLmalloc(MEM_CHUNK_POOL* _this, size_t Size,
                           gsi_i32 Alignment); //= PTR_ALIGNMENT);
// allocated backwards from top of heap
void* MEM_CHUNK_POOLmalloc_backwards(MEM_CHUNK_POOL* _this, size_t Size,
                                     gsi_i32 Alignment); //= PTR_ALIGNMENT);
void* MEM_CHUNK_POOLrealloc(MEM_CHUNK_POOL* _this, void* oldmem,
                            size_t newSize);
...
gsi_u32 MEM_CHUNK_POOLWalkForType(MEM_CHUNK_POOL* _this, int _MemType,
                                  gsi_bool _LogUse);
...
void MEM_CHUNK_POOLFillMemoryTable(MEM_CHUNK_POOL* _this, char* Table,
                                   const int TableSize, gsi_u32 _HeapStart,
                                   gsi_u32 _HeapSize);
...
//--------------------------------------------------------------------------
{
  ...
  MP_ASSERT((gsi_uint)newpos <= (gsi_uint)oldpos->next -
                                    MEM_CHUNKMemUsedGet(oldpos) -
                                    sizeof(MEM_CHUNK))
  MP_ASSERT((gsi_uint)newpos >= (gsi_uint)oldpos->prev +
                                    MEM_CHUNKMemUsedGet(oldpos->prev) +
                                    sizeof(MEM_CHUNK))
  ...
  *newpos = temp;
  ...
  if (MEM_CHUNKIsFree(newpos)) {
    ...
    if (_this->pFirstFree == oldpos)
      ...
      MP_ASSERT((newpos->NextFree == NULL) ||
                ((gsi_uint)newpos->NextFree > (gsi_uint)newpos))
    }
  }
}
...
//--------------------------------------------------------------------------
void MEM_CHUNK_POOLCreate(MEM_CHUNK_POOL* _this, const char* szNameIn,
...
//--------------------------------------------------------------------------
{
  ...
  MEM_CHUNKAlloc(_this->HeaderStart, 0,
                 sizeof(MEM_CHUNK)); // don't mark as free
...
// it. return header if start header is first free chunk
{
  while ((header) && (!MEM_CHUNKIsFree(header))) {
    // GS_ASSERT(header->prev == NULL || (header->prev >= _this->HeaderStart &&
...
//--------------------------------------------------------------------------
{
  ...
  NewHeader = (MEM_CHUNK*)((gsi_u8*)header + MEM_CHUNKMemUsedGet(header) +
                           sizeof(MEM_CHUNK));
  NewHeader = (MEM_CHUNK*)MEMALIGN_POWEROF2(NewHeader, sizeof(MEM_CHUNK));

  // assert we have enough room for this new chunk
  MP_ASSERT((gsi_uint)NewHeader + 2 * sizeof(MEM_CHUNK) <=
            (gsi_uint)header->next)

// update some stats
#if (MEM_PROFILE)
  if (ReAlloc) {
    // 09-OCT-07 BED: Since we're splitting the chunk, it seems more accurate
    //               to use the full size of the chunk, not just the used
    //               portion
  ...
  // create a new chunk header, at the end of used space, plus enough to align
  // us to 16 bytes
  ...
  if (MEM_CHUNKIsFree(NewHeader->next)) {
    ...
    if (ReAlloc) {
      ...
      NewHeader->NextFree =
          MEM_CHUNK_POOLFindNextFreeChunk(_this, NewHeader->next);
    } else
...
//--------------------------------------------------------------------------
MEM_CHUNK* MEM_CHUNK_POOLAllocChunk(MEM_CHUNK_POOL* _this, size_t Size,
                                    gsi_i32 Alignment, gsi_bool Backwards)
...
//--------------------------------------------------------------------------
{
  ...
  SizeNeeded = MEMALIGN_POWEROF2(
      SizeNeeded, sizeof(MEM_CHUNK)); // must be aligned to this at least!!!
  ...
  while (header) {
    ...
    if (MemRemain >= 0) {
      ...
      // at this point we've taken this chunk, and need to split off the unused
      // part in theory, there should be no other free chunk ahead of us.
...
//--------------------------------------------------------------------------
{
  ...
  gsi_u32 destptr =
      (gsi_u32)prev->prev + prev->prev->MemAvailGet() + sizeof(MEM_CHUNK);
...
//--------------------------------------------------------------------------
{
  ...
  MEM_CHUNK* header =
      MEM_CHUNK_POOLAllocChunk(_this, Size, Alignment, gsi_false);
...
//--------------------------------------------------------------------------
{
  ...
  // return ptr to the first block big enough
  MEM_CHUNK* header =
      MEM_CHUNK_POOLAllocChunk(_this, Size, Alignment, gsi_true);
...
//--------------------------------------------------------------------------
{
  ...
  if (newSize < OldSize) {
    ...
    NewHeader =
        MEM_CHUNK_POOLAllocChunk(_this, newSize, PTR_ALIGNMENT, gsi_false);
...
//--------------------------------------------------------------------------
gsi_u32 MEM_CHUNK_POOLWalkForType(MEM_CHUNK_POOL* _this, int type,
                                  gsi_bool _LogUse)
//--------------------------------------------------------------------------
{
  MEM_CHUNK* header;
  ...
  while (header) {
    MP_ASSERT((header->next == NULL) ||
              ((gsi_uint)header <
               (gsi_uint)header->next)) // infinite loop or out of place
    MP_ASSERT((header->prev == NULL) ||
              ((gsi_uint)header->prev <
               (gsi_uint)header)) // infinite loop or out of place
    MP_ASSERT((header->prev == NULL) ||
              (header->prev->next == header)) // previous linked correctly to us
    MP_ASSERT(
        (header->next == NULL) ||
        (header->next->prev == header)) // next		linked correctly to us
    MP_ASSERT(MEM_CHUNKMemUsedGet(header) <=
              MEM_CHUNKChunkSizeGet(header)) // using too much mem
    ...
    if (!MEM_CHUNKIsFree(header) && (MEM_CHUNKMemTypeGet(header) == type)) {
      ...
      if ((header != _this->HeaderStart) && (header != _this->HeaderEnd)) {
        ...
        if (_LogUse) {
          gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc,
...
void MEM_CHUNK_POOLMemStatsGet(MEM_CHUNK_POOL* _this, MEM_STATS* pS) {
  ...
  while (header) {
    ...
    if (!MEM_CHUNKIsFree(header)) {
      ...
      pS->MemWasted +=
          MEM_CHUNKTotalSizeGet(header) - MEM_CHUNKMemUsedGet(header);
      ...
      MP_ASSERT((header->NextFree == NULL) ||
                ((gsi_uint)header <
                 (gsi_uint)header->NextFree)) // infinite loop or out of place

      // make sure we aren't fragmented, as this ruins some algorithm
      // assumptions
      MP_ASSERT(
          (header->next == NULL) ||
          (!MEM_CHUNKIsFree(header->next))) // infinite loop or out of place
      MP_ASSERT(
          (header->prev == NULL) ||
          (!MEM_CHUNKIsFree(header->prev))) // infinite loop or out of place

      // previous free chunk linked correctly to us, we aren't a lost chunk
      MP_ASSERT(header == NextFree)
      NextFree = header->NextFree;

      // calc overhead and waste (in this case, the same
      // value...sizeof(MEM_CHUNK) header)
      pS->MemWasted +=
          MEM_CHUNKTotalSizeGet(header) - MEM_CHUNKChunkSizeGet(header);
      pS->MemUsed +=
          MEM_CHUNKTotalSizeGet(header) - MEM_CHUNKChunkSizeGet(header);
...
{
  int i;
  // find which pool owns this pointer!!!!, this is kind of a hack.... but here
  // goes.
  for (i = 0; i < gsMemMgrContext_Count; i++) {
    if (MEM_CHUNK_POOLIsValid(&gChunkPool[i]) &&
...
void* gs_malloc(size_t size) {
  GS_ASSERT(size)
  GS_ASSERT_STR(MEM_CHUNK_POOLIsValid(&gChunkPool[gsMemMgrContextCurrent]),
                "malloc: context is invalid mempool");

  return MEM_CHUNK_POOLmalloc(&gChunkPool[gsMemMgrContextCurrent], size,
                              PTR_ALIGNMENT);
}
...
void* gs_calloc(size_t size, size_t size2) {
  ...
  return MEM_CHUNK_POOLmalloc(&gChunkPool[gsMemMgrContextCurrent], size * size2,
                              PTR_ALIGNMENT);
}
...
void gs_free(void* ptr) {
  ...
  GS_ASSERT_STR(context != gsMemMgrContext_Invalid,
...
// Create a mem pool for the given context.  If that context is in use, it will
// return the next available if none are available it will return
// gsMemMgrContext_Invalid ex use:  gQR2MemContext = gsMemMgrCreate
// (0,0,16 * 1024); will find the first avaiable spot, create a mem pool of 16k,
...
gsMemMgrContext gsMemMgrCreate(gsMemMgrContext context, const char* PoolName,
  ...
  GS_ASSERT_STR(((((gsi_uint)thePoolSize) & 15) == 0),
                "PoolSize	must be aligned to 16 bytes");
  GS_ASSERT_STR(((((gsi_uint)thePoolBuffer) & 15) == 0),
...
void gsMemMgrContextPush(gsMemMgrContext NewType) {
  ...
  //	gsDebugFormat(GSIDebugCat_App, GSIDebugType_State,
  // GSIDebugLevel_Comment,"MemProfilerStart:
...
gsMemMgrContext gsMemMgrContextPop() {
  ...
  //		gsDebugFormat(GSIDebugCat_App, GSIDebugType_State,
  // GSIDebugLevel_Comment,"MemProfilerEnd:
...
gsi_u32 gsMemMgrMemAvailGet(gsMemMgrContext context) {
  ...
  GS_ASSERT_STR(context < gsMemMgrContext_Count,
...
//--------------------------------------------------------------------------
{
  ...
  char* ptr =
      (char*)(((gsi_uint)malloc(size - PTR_ALIGNMENT) + (PTR_ALIGNMENT - 1)) &
              ~(PTR_ALIGNMENT - 1));
  ...
  while (1) {
    ...
    if ((i == 0) && (PtrTableCount < 1024)) {
      ...
      if (c & 1)
        ...
        PtrTable[PtrTableCount] =
            MEM_CHUNK_POOLmalloc_backwards(&gChunkPool, j, k);
