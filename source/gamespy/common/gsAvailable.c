...
{
        sendto(AC.sock, AC.packet, AC.packetLen, 0, (SOCKADDR *)&AC.address,
sizeof(AC.address)); AC.sendTime = current_time();
}
...
void GSIStartAvailableCheckA(const char * gamename)
{
        ...
        rcode = get_sockaddrin(override?GSIACHostname:hostname, MASTER_PORT,
&AC.address); if(!rcode) return;
...
static int HandlePacket(char * packet, int len, SOCKADDR_IN * address, int *
disabledservices)
{
        int bitfield;
        ...
        // check the IP
        if(memcmp(&address->sin_addr, &AC.address.sin_addr, sizeof(IN_ADDR)) !=
0) return 1;
...
GSIACResult GSIAvailableCheckThink(void)
{
        ...
        // if we don't have a sock, possibly because of an initialization error,
default to available if(AC.sock == INVALID_SOCKET)
        {
        ...
        if(CanReceiveOnSocket(AC.sock))
        {
                ...
                rcode = (int)recvfrom(AC.sock, packet, (int)sizeof(packet), 0,
(SOCKADDR *)&address, &len);

                // verify the packet
                rcode = HandlePacket(packet, rcode, &address,
&disabledservices); if(rcode == 0)
                {
