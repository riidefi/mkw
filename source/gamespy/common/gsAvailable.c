...
void GSIStartAvailableCheckA(const char* gamename) {
  ...
  // setup the hostname
  override_ = GSIACHostname[0];
  if (!override_)
    sprintf(hostname, "%s.available." GSI_DOMAIN_NAME, gamename);

  // get the master address
  rcode = get_sockaddrin(override_ ? GSIACHostname : hostname, MASTER_PORT,
                         &AC.address);
  if (!rcode)
    return;
...
static int HandlePacket(char* packet, int len, SOCKADDR_IN* address,
