...
int gsUdpMsgHandlerCompare(const void* theFirstHandler,
                           const void* theSecondHandler) {
  GSUdpMsgHandler *msgHandler1 = (GSUdpMsgHandler*)theFirstHandler,
                  *msgHandler2 = (GSUdpMsgHandler*)theSecondHandler;
  int initCmp;
  initCmp = memcmp(msgHandler1->mInitialMsg, msgHandler2->mInitialMsg,
                   GS_UDP_MSG_HEADER_LEN);
  return initCmp;
}

// Used to find a message handler based on the header.
int gsUdpMsgHandlerCompare2(const void* theFirstHandler,
                            const void* theSecondHandler) {
  GSUdpMsgHandler *msgHandler1 = (GSUdpMsgHandler*)theFirstHandler,
                  *msgHandler2 = (GSUdpMsgHandler*)theSecondHandler;
  int headerCmp;
  headerCmp =
      memcmp(msgHandler1->mHeader, msgHandler2->mHeader, GS_UDP_MSG_HEADER_LEN);
  return headerCmp;
}
...
int gsUdpRemotePeerCompare(const void* theFirstPeer,
                           const void* theSecondPeer) {
...
int gsUdpRemotePeerCompare2(const void* theFirstPeer,
                            const void* theSecondPeer) {
  GSUdpRemotePeer *aPeer1 = (GSUdpRemotePeer*)theFirstPeer,
                  *aPeer2 = (GSUdpRemotePeer*)theSecondPeer;
...
void gsUdpSocketError(GT2Socket theSocket) {
  ...
  for (i = 0; i < len; i++) {
    GSUdpMsgHandler* aMsgHandler =
        (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, i);
...
void gsUdpClosedRoutingCB(GT2Connection theConnection, GT2CloseReason reason) {
  ...
  for (index = 0; index < len; index++) {
    GSUdpMsgHandler* aHandler =
        (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, index);
    if (aHandler->mClosed) {
      gsDebugFormat(
          GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment,
          "[Udp Engine] Connection closed: passed to message handler\n");
      aHandler->mClosed(gt2GetRemoteIP(theConnection),
                        gt2GetRemotePort(theConnection), aReason,
                        aHandler->mUserData);
    }
  }
  ...
  if (aUdp->mAppClosed) {
    ...
    aUdp->mAppClosed(gt2GetRemoteIP(theConnection),
                     gt2GetRemotePort(theConnection), aReason,
                     aUdp->mAppUserData);
  }

  aRemotePeer.mConnection = theConnection;
  index = ArraySearch(aUdp->mRemotePeers, &aRemotePeer, gsUdpRemotePeerCompare2,
                      0, 0);
  if (index != NOT_FOUND) {
...
void gsUdpConnectedRoutingCB(GT2Connection theConnection, GT2Result theResult,
  ...
  if (theResult == GT2Rejected) {
    ...
    aRemotePeerIdx = ArraySearch(aUdp->mRemotePeers, &aRemotePeer,
                                 gsUdpRemotePeerCompare, 0, 0);
  ...
  for (aIndex = 0; aIndex < len; aIndex++) {
    int aRemotePeerIdx;
    GSUdpRemotePeer aRemotePeer;
    GSUdpMsgHandler* aTempHandler =
        (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, aIndex);

    aRemotePeer.mAddr = gt2GetRemoteIP(theConnection);
    aRemotePeer.mPort = gt2GetRemotePort(theConnection);
    aRemotePeerIdx = ArraySearch(aTempHandler->mPendingConnections,
                                 &aRemotePeer, gsUdpRemotePeerCompare, 0, 0);
    if (aRemotePeerIdx != NOT_FOUND) {
      if (aTempHandler->mConnected) {
        ...
        aTempHandler->mConnected(
            ...
            aCode, theResult == GT2Rejected ? gsi_true : gsi_false,
...
void gsUdpPingRoutingCB(GT2Connection theConnection, int theLatency) {
  ...
  if (aUdp->mAppPingReply) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
                  GSIDebugLevel_Comment, "[Udp Engine] Passed to app\n");
...
void gsUdpReceivedRoutingCB(GT2Connection theConnection, GT2Byte* theMessage,
                            int theMessageLen, GT2Bool reliable) {
  GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
  GSUdpMsgHandler aHandler;
  int index;
  char anAddr[GS_IP_ADDR_AND_PORT];
  ...
  if (theMessageLen >= GS_UDP_MSG_HEADER_LEN) {
    ...
    index = ArraySearch(aUdp->mMsgHandlers, &aHandler, gsUdpMsgHandlerCompare2,
                        0, 0);
    if (index != NOT_FOUND) {
      GSUdpMsgHandler* aHandlerFound =
          (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, index);
      if (aHandlerFound->mReceived) {
        ...
        aHandlerFound->mReceived(
            ...
            (unsigned int)(theMessageLen - GS_UDP_MSG_HEADER_LEN), reliable,
...
GT2Bool gsUdpUnrecognizedMsgCB(GT2Socket theSocket, unsigned int theIp,
  ...
  if (aUdp->mAppUnknownMessage) {
    ...
    aRet =
        aUdp->mAppUnknownMessage(theIp, thePort, (unsigned char*)theMessage,
...
void gsUdpConnAttemptCB(GT2Socket socket, GT2Connection connection,
  ...
  // If there is a handler, automatically accept a connection if the initial
  // message is the same as the handler's registered initial message
  if (len >= GS_UDP_MSG_HEADER_LEN) {
    ...
    index = ArraySearch(aUdp->mMsgHandlers, &aHandler, gsUdpMsgHandlerCompare,
                        0, 0);
    if (index != NOT_FOUND) {
      ...
      gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
                    GSIDebugLevel_Comment,
                    "[Udp Engine] Connection attempt auto-accepted for message "
                    "handler\n");
      return;
    }
  }
  ...
  if (aUdp->mAppConnAttempt) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
                  GSIDebugLevel_Comment,
                  "[Udp Engine] Connection attempt from %s, asking app to "
                  "accept/reject\n",
                  gt2AddressToString(ip, port, anAddr));
    aUdp->mAppConnAttempt(ip, port, latency, (unsigned char*)message,
                          (unsigned int)len, aUdp->mAppUserData);
  } else {
...
GSUdpErrorCode gsUdpEngineInitialize(
    unsigned short thePort, int theIncomingBufSize, int theOutgoingBufSize,
    gsUdpErrorCallback theAppNetworkError,
    gsUdpConnConnectedCallback theAppConnected,
    gsUdpConnClosedCallback theAppClosed, gsUdpConnPingCallback theAppPing,
    gsUdpConnReceivedDataCallback theAppReceive,
    gsUdpUnknownMsgCallback theAppUnownMsg,
    gsUdpAppConnectAttemptCallback theAppConnectAttempt, void* theAppUserData) {
  ...
  incomingBufferSize =
      theIncomingBufSize != 0 ? theIncomingBufSize : GS_UDP_DEFAULT_IN_BUFFSIZE;
  outgoingBufferSize = theOutgoingBufSize != 0 ? theOutgoingBufSize
                                               : GS_UDP_DEFAULT_OUT_BUFFSIZE;

  // Setup our internal socket that will be shared among more than one
  // application
  ...
  aGt2Result = gt2CreateSocket(&aUdp->mSocket, anAddr, outgoingBufferSize,
                               incomingBufferSize, gsUdpSocketError);
  ...
  // We'll need to keep track of connections with an array of GPconnection to
  // address mapping
  aUdp->mRemotePeers = ArrayNew(sizeof(GSUdpRemotePeer), 1, NULL);
  if (aUdp->mRemotePeers == NULL) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Memory,
                  GSIDebugLevel_HotError, "[Udp Engine] No more memory!!!\n");
    return GS_UDP_NO_MEMORY;
  }

  aUdp->mMsgHandlers =
      ArrayNew(sizeof(GSUdpMsgHandler), 1, gsUdpMsgHandlerFree);
  ...
  // Used by the manager to receive messages from backend services or other
  // clients that may not be using gt2
  gt2SetUnrecognizedMessageCallback(aUdp->mSocket, gsUdpUnrecognizedMsgCB);

  // Automatically start listening for all SDKS and the game if game uses UDP
  // Engine
...
GSUdpErrorCode gsUdpEngineGetPeerState(unsigned int theIp,
  ...
  *thePeerState =
      (GSUdpPeerState)gt2GetConnectionState(aPeerFound->mConnection);
  return GS_UDP_NO_ERROR;
}
...
// IP and port.
GSUdpErrorCode
gsUdpEngineStartTalkingToPeer(unsigned int theIp, unsigned short thePort,
  ...
  index = ArraySearch(aUdp->mRemotePeers, &aRemotePeer, gsUdpRemotePeerCompare,
                      0, 0);
  if (index != NOT_FOUND) {
    GSUdpRemotePeer* aPeerFound =
        (GSUdpRemotePeer*)ArrayNth(aUdp->mRemotePeers, index);
    ...
    // start the connect without blocking since we want the engine to be as
    // asynchronous as possible
    gt2Connect(aUdp->mSocket, &aRemotePeer.mConnection, anAddr,
               (unsigned char*)theInitMsg, GS_UDP_MSG_HEADER_LEN, timeOut,
               &aCallbacks, GT2False);
    ...
    index = ArraySearch(aUdp->mMsgHandlers, &aHandler, gsUdpMsgHandlerCompare,
                        0, 0);
    if (index != NOT_FOUND) {
      GSUdpRemotePeer* aRemotePeerPtr = (GSUdpRemotePeer*)ArrayNth(
          aUdp->mRemotePeers, ArrayLength(aUdp->mRemotePeers) - 1);
      GSUdpMsgHandler* aHandlerFound =
          (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, index);
...
GSUdpErrorCode gsUdpEngineAcceptPeer(unsigned int theIp,
                                     unsigned short thePort) {
  GSUdpRemotePeer aRemotePeer;
  GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
  int index;
...
// Should only be used by App
GSUdpErrorCode gsUdpEngineRejectPeer(unsigned int theIp,
                                     unsigned short thePort) {
  GSUdpRemotePeer aRemotePeer;
  GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
  int index;
...
// theIp and thePort cannot be 0 (Zero)
//
// WARNING: Messages should not be greater than the outgoing buffer size minus
// the header and the 7 byte header for reliable messages (used for internal gt2
// operations). Most UDP fragmentation occurs if messages are bigger than 1500
// bytes.  Also, some routers are known to drop those packets that are larger
// than 1500 bytes.  The recommended outgoing buffer size is the default (1460).
// So take that, and subtract 16 for message handler header and reliable message
// header (if sending data reliably). freeSpace = 1460 - 16 - 7
GSUdpErrorCode gsUdpEngineSendMessage(unsigned int theIp,
  ...
  if (aTotalMessageLen >
          gt2GetOutgoingBufferSize(aRemotePeerFound->mConnection) &&
      theReliable) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
  ...
  aResult = gt2Send(aRemotePeerFound->mConnection, fullMessage,
                    aTotalMessageLen, theReliable);
  gsifree(fullMessage);
...
GSUdpErrorCode
gsUdpEngineAddMsgHandler(char theInitMsg[GS_UDP_MSG_HEADER_LEN],
                         char theHeader[GS_UDP_MSG_HEADER_LEN],
  ...
  // setup a message handler that the UDP engine will use to pass connection
  // attempts to
...
////////////////////////////////////////////////////////////////////////////////
// UDP Layer must be initialized
// When a message handler is done or shutting down, the message handler should
// remove itself from the UDP Layer The header cannot be empty
GSUdpErrorCode
gsUdpEngineRemoveMsgHandler(char theHeader[GS_UDP_MSG_HEADER_LEN]) {
  GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
  GSUdpMsgHandler aHandler;
  int index;
  ...
  index =
      ArraySearch(aUdp->mMsgHandlers, &aHandler, gsUdpMsgHandlerCompare2, 0, 0);
  if (index != NOT_FOUND) {
...
gsi_bool gsUdpEngineNoApp() {
  ...
  if (aUdp->mAppClosed || aUdp->mAppConnAttempt || aUdp->mAppConnected ||
      aUdp->mAppNetworkError || aUdp->mAppPingReply || aUdp->mAppRecvData ||
      aUdp->mAppUnknownMessage) {
    return gsi_false;
  }
...
// of a peer's buffer.
int gsUdpEngineGetPeerOutBufferFreeSpace(unsigned int theIp,
                                         unsigned short thePort) {
  GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
  GSUdpRemotePeer aRemotePeer, *aRemotePeerFound;
  int index;
