...
//#define GS_COMMON_DEBUG      // use GameSpy debug utilities for SDK debug
// output #define GS_WINSOCK2          // use winsock2 #define GS_UNICODE // Use
// widechar (UCS2) SDK interface. #define GS_NO_FILE           // disable file
// storage (on by default for PS2/DS) #define GS_NO_THREAD         // no
// multi-thread support
//#define GS_PEER		       // MUST be defined if you are using Peer
// SDK
...
#define GSI_LIMIT(x, minx, maxx)                                               \
  (((x) < (minx) ? (minx) : ((x) > (maxx) ? (maxx) : (x))))
#define GSI_WRAP(x, minx, maxx)                                                \
  (((x) < (minx) ? (maxx - 1) : ((x) >= (maxx) ? (minx) : (x))))
