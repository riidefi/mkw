...
//  sending accross the net
//	And on big endian machines, convert little endian to big endian on recv
//#define	_GT2_ENDIAN_CONVERT_ENABLE	// add this to your compiler
...
// application will not use, or not use frequently. the only impact of this
// change will be to make your application incomatible with other application's
// using either the original or another different magic string.
// the string can consist of any number of characters, as long as there's at
// least one character, and the length define matches the string's length.
...
// buffer is declared on the stack, and so can be fairly large on most systems
// without having any impact.  however, on some systems with small stacks, this
// size can overflow the stack, in which case it should be lowered. note, this
...
typedef struct GTI2Socket {
  ...
  gt2ConnectAttemptCallback
      connectAttemptCallback; // if set, callback used to handle incoming
                              // connection attempts
  gt2SocketErrorCallback
      socketErrorCallback; // if set, call this in case of an error
  gt2DumpCallback
      sendDumpCallback; // if set, gets called for every datagram sent
  ...
  gt2UnrecognizedMessageCallback
      unrecognizedMessageCallback; // if set, gets called for all unrecognized
                                   // messages
  ...
  GTI2ProtocolType
      protocolType; // set to UDP or VDP protocol depending on the call to
...
typedef struct GTI2Connection {
  ...
  unsigned int
      ip; // the ip on the other side of this connection (network byte order)
  unsigned short
      port; // the port on the other side of this connection (host byte order)
  ...
  unsigned short
      serialNumber; // serial number of the next message to be sent out
