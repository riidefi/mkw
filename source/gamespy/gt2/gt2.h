...
/*
** see "configurable defines" in gt2Main.h for certain performance settings that
*can be changed
*/
...
// 64K for PC, 4k for Xbox). the buffer needs to be able to hold all messages
// waiting for confirmation of delivery, and it needs to hold any messages that
// arrive out of order. if either buffer runs out of space the connection will
// be dropped.
GT2Result gt2CreateSocket(
    GT2Socket* socket, // if the result is GT2Success, the socket object handle
                       // will be stored at this address
    const char* localAddress, // the local address to bind to
    int outgoingBufferSize, // size of per-connection buffer where sent messages
                            // waiting to be confirmed are held, use 0 for
                            // default
    int incomingBufferSize, // size of per-connection buffer where out-of-order
                            // received messages are held, use 0 for default
    gt2SocketErrorCallback callback // a callback that is called if there is an
...
// gt2CloseAllConnectionsHard was called for this socket.  all connections send
// a close message to the remote side, and any closed callbacks will be called
...
GT2Result gt2SendRawUDP(
    ...
    const GT2Byte*
        message, // the message to send, or NULL for an empty datagram
    int len      // the len of the message (0 for an empty message, ignored if
                 // message==NULL)
);
...
typedef struct {
  ...
  gt2ClosedCallback
      closed; // Called when the connection is closed (remotely or locally).
...
// if blocking is false, the return value signals the current status of the
// attempt
//   GT2Success means the connection is being attempted
//   anything else means there was an error and the connection attempt has been
//   aborted
GT2Result gt2Connect(
    GT2Socket socket, // the local socket to use for the connection
    GT2Connection*
        connection, // if the result is GT2Success, and blocking is false, the
                    // connection  object handle is stored here
    ...
    GT2Bool blocking // if true, don't return until complete (successfuly or
                     // unsuccessfuly)
);

// sends data reliably or unreliably
// reliable messages are guaranteed to arrive, arrive in order, and arrive only
// once. unreliable messages are not guaranteed to arrive, arrive in order, or
// arrive only once. because messages may be held in the outgoing buffer (even
// unreliable messages may need to be put in the buffer), the message size
// cannot exceed
GT2Result
gt2Send(GT2Connection connection, // the connection to send the message on
        const GT2Byte*
            message, // the message to send, or NULL for an empty message0
        int len, // the len of the message (0 for an empty message, ignored if
                 // message==NULL)
...
// same as gt2CloseConnection, but doesn't wait for confirmation from the remote
// side of the connection the closed callback will be called from within this
// function
void gt2CloseConnectionHard(GT2Connection connection);

// closes all of a socket's connections (essentially calls gt2CloseConnection on
// each of them).
...
typedef void (*gt2ConnectAttemptCallback)(
    ...
    GT2Connection
        connection,  // a connection object for the incoming connection attempt
    unsigned int ip, // the IP being used remotely for the connection attempt
    unsigned short
        port,         // the port being used remotely for the connection attempt
...
// if the socket is already listening, this callback will replace the exsiting
// callback being used if the callback is NULL, this will cause the connection
// to stop listening
void gt2Listen(GT2Socket socket, gt2ConnectAttemptCallback callback);

// after a socket's gt2ConnectAttemptCallback has been called, this function can
// be used to accept the incoming connection attempt.  it can be called from
// either within the callback or some later time. as soon as it is called the
// connection is active, and messages can be sent and received.  the remote side
// of the connection will have it's connected callback called with the result
// set to GT2Success.  the callbacks that are passed in to this function are the
// same callbacks that get passed to gt2Connect, with the exception that the
// connected callback can be ignored, as the connection is already established.
// if this function returns GT2True, then the connection has been successfully
// accepted.  if it returns GT2False, then the remote side has already closed
// the connection attempt.  in that case, the connection is considered closed,
// and it cannot be referenced again.
GT2Bool gt2Accept(GT2Connection connection, GT2ConnectionCallbacks* callbacks);

// after a socket's gt2ConnectAttemptCallback has been called, this function can
// be used to reject the incoming connection attempt.  it can be called from
// either within the callback or some later time. once the function is called
// the connection is considered closed and cannot be referenced again.  the
// remote side attempting the connection will have its connected callback called
// with the result set to GT2Rejected. if the message is not NULL and the len is
// not 0, the message will be sent with the rejection, and passed into the
// remote side's connected callback.
void gt2Reject(GT2Connection connection, const GT2Byte* message, int len);

/*************************
** MESSAGE CONFIRMATION **
*************************/
// gets the message id for the last reliably sent message. unreliable messages
// do not have ids. this should be called immediately after gt2Send.  waiting
// until after a call to gt2Think can result in an invalid message id being
// returned. note that the use of filters that can either drop or delay messages
// can complicate the process, because in those cases a call to gt2Send does not
// guarantee that a message will actually be sent.  in those cases,
// gt2GetLastSentMessageID should be called after gt2FilteredSend, because the
// actual message will be sent from within that function.
GT2MessageID gt2GetLastSentMessageID(GT2Connection connection);

// returns true if confirmation was received locally that the reliable message
// represented by the message id was received by the remote end of the
// connection.  returns false if confirmation was not yet received. this should
// only be called on message ids that were returned by gt2GetLastSendMessageID,
// and should be used relatively soon after the message was sent, due to message
// ids wrapping around after a period of time.
GT2Bool gt2WasMessageIDConfirmed(GT2Connection connection,
...
typedef void (*gt2SendFilterCallback)(
    GT2Connection
        connection, // The connection on which the message is being sent.
    int filterID,   // Pass this ID to gt2FilteredSend.
    const GT2Byte*
        message, // The message being sent.  Will be NULL if an empty message.
...
// Returns GT2False if there was an error adding the filter (due to no free
// memory)
GT2Bool gt2AddSendFilter(
    GT2Connection connection, // The connection on which to add the filter.
    gt2SendFilterCallback
        callback // The callback the outgoing data is filtered through.
);
...
GT2Bool gt2AddReceiveFilter(
    ...
    gt2ReceiveFilterCallback
        callback // The callback the incoming data is filtered through.
);
...
// sets a callback that all unrecognized messages are passed to.  an
// unrecognized message is one that can't be matched up to a specific
// connection. if the callback handles the message, it returns true, and the
// GT2Socket ignores the message.  if the callback does not recognize the
// message, it returns false, and the socket handles the message (by sending a
// message back indicating the connection is closed).  if the callback is NULL,
// it removes any previously set callback.
void gt2SetUnrecognizedMessageCallback(GT2Socket socket,
...
// GT2Connected - the connection is active (has successfully connected, and not
// yet closed) GT2Closing - the connection is in the process of closing (i.e.,
// sent a close message and waiting for confirmation). GT2Closed - the
// connection has already been closed and will soon be freed
...
// Converts an IP and a port into a text string.  The IP must be in network byte
// order, and the port in host byte order.  The string must be able to hold at
// least 22 characters (including the NUL). "XXX.XXX.XXX.XXX:XXXXX" If both the
// IP and port are non-zero, the string will be of the form "1.2.3.4:5"
// ("<IP>:<port>"). If the port is zero, and the IP is non-zero, the string will
// be of the form "1.2.3.4" ("<IP>"). If the IP is zero, and the port is
// non-zero, the string will be of the form ":5" (":<port>"). If both the IP and
// port are zero, the string will be an empty string ("") The string is
// returned.  If the string paramater is NULL, then an internal static string
// will be used.  There are two internal strings that are alternated between.
...
// Converts a string address into an IP and a port.  The IP is stored in network
// byte order, and the port is stored in host byte order.  Returns false if
// there was an error parsing the string, or if a supplied hostname can't be
// resolved. Possible string forms: NULL => all IPs, any port (localAddress
// only).
...
// If this function needs to resolve a hostname ("host.com") it may need to
// contact a DNS server, which can cause the function to block for an indefinite
// period of time.  Usually it is < 2 seconds, but on certain systems, and under
// certain circumstances, it can take 30 seconds or longer.
GT2Bool gt2StringToAddress(
    const char* string, // The string to convert.
    unsigned int*
        ip, // The IP is stored here, in network byte order.  Can be NULL.
    unsigned short*
        port // The port is stored here, in host byte order.  Can be NULL.
);

// Gets the host information for a machine on the Internet.  The first version
// takes an IP in network byte order, and the second version takes a string that
// is either a dotted ip ("1.2.3.4"), or a hostname ("www.gamespy.com"). If the
// function can successfully lookup the host's info, the host's main hostname
// will be returned.  If it cannot find the host's info, it returns NULL. For
// the aliases parameter, pass in a pointer to a variable of type (char **).  If
// this parameter is not NULL, and the function succeeds, the variable will
// point to a NULL-terminated list of alternate names for the host. For the ips
// parameter, pass in a pointer to a variable of type (int **).  If this
// parameter is not NULL, and the function succeeds, the variable will point to
// a NULL-terminated list of altername IPs for the host.  Each element in the
// list is actually a pointer to an unsigned int, which is an IP address in
// network byte order. The return value, aliases, and IPs all point to an
// internal data structure, and none of these values should be modified
// directly.  Also, the data is only valid until another call needs to use the
// same data structure (virtually ever internet address function will use this
// data structure). If the data will be needed in the future, it should be
// copied off. If this function needs to resolve a hostname ("host.com") it may
// need to contact a DNS server, which can cause the function to block for an
...
// (gt2*ToHostInfo()), and each performs a subset of the functionality.  They
// are provided so that code that only needs certain information can be a little
// simpler. Before using these, read the comments for the gt2*ToHostInfo()
// functions, as the info also applies to these functions.
...
// these are for getting around adhoc which requires a 48 bit address v.s. a 32
// bit inet address
...
// sets a callback to be called whenever a UDP datagram is sent or received, and
// when a connection reset is received. pass in a callback of NULL to remove the
// callback.  the dumps sit at a lower level than the filters, and allow an app
// to keep an eye on exactly what datagrams are being sent and received,
// allowing for close monitoring.  however the dumps cannot be used to modify
// data, only monitor it.  the dumps are useful for debugging purposes, and to
// keep track of data send and receive rates (e.g., the Quake 3 engine's
// netgraph). note that these are the actual UDP datagrams being sent and
// received - datagrams may be dropped, repeated, or out-of-order.  control
// datagrams (those used internally by the protocol) will be passed to the dump
// callbacks, and certain application messages will have a header at the
// beginning.
