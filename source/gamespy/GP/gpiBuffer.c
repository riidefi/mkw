...
GPResult gpiAppendStringToBufferLen(GPConnection* connection,
                                    GPIBuffer* outputBuffer, const char* string,
                                    int stringLen) {
  int len;
  int size;
  char* output;
...
GPResult gpiAppendStringToBuffer(GPConnection* connection,
                                 GPIBuffer* outputBuffer, const char* buffer) {
  return gpiAppendStringToBufferLen(connection, outputBuffer, buffer,
                                    (int)strlen(buffer));
}

GPResult gpiAppendShortToBuffer(GPConnection* connection,
                                GPIBuffer* outputBuffer, short num) {
  char shortVal[8];
  sprintf(shortVal, "%d", num);
  return gpiAppendStringToBuffer(connection, outputBuffer, shortVal);
}

GPResult gpiAppendUShortToBuffer(GPConnection* connection,
                                 GPIBuffer* outputBuffer, unsigned short num) {
  char shortVal[8];
  sprintf(shortVal, "%u", num);
  return gpiAppendStringToBuffer(connection, outputBuffer, shortVal);
}

GPResult gpiAppendIntToBuffer(GPConnection* connection, GPIBuffer* outputBuffer,
                              int num) {
  char intValue[16];
...
GPResult gpiAppendUIntToBuffer(GPConnection* connection,
                               GPIBuffer* outputBuffer, unsigned int num) {
  char intValue[16];
...
static GPResult gpiSendData(GPConnection* connection, SOCKET sock,
                            const char* buffer, int bufferLen, GPIBool* closed,
                            int* sent, char id[3]) {
  int rcode;
...
GPResult gpiSendOrBufferStringLenToPeer(GPConnection* connection,
                                        GPIPeer_st peer, const char* string,
                                        int stringLen) {
  ...
  if (!(peer->outputBuffer.len - peer->outputBuffer.pos) &&
      !ArrayLength(peer->messages)) {
    if ((int)remaining <=
        (gsUdpEngineGetPeerOutBufferFreeSpace(peer->ip, peer->port) -
         GS_UDP_RELIABLE_MSG_HEADER - GS_UDP_MSG_HEADER_LEN)) {
      gsUdpEngineSendMessage(peer->ip, peer->port, iconnection->mHeader,
                             (unsigned char*)string, remaining, gsi_true);
      total = remaining;
      remaining = 0;
    } else {
      unsigned int freeSpace =
          (unsigned int)gsUdpEngineGetPeerOutBufferFreeSpace(peer->ip,
                                                             peer->port);
      if (freeSpace > (GS_UDP_MSG_HEADER_LEN + GS_UDP_RELIABLE_MSG_HEADER)) {
        ...
        gsUdpEngineSendMessage(peer->ip, peer->port, iconnection->mHeader,
                               (unsigned char*)string, sent, gsi_true);
        total = sent;
        remaining -= sent;
      }
    }
  ...
  ///////////////////////////
  if (remaining)
    CHECK_RESULT(gpiAppendStringToBufferLen(connection, &peer->outputBuffer,
                                            &string[total], (int)remaining));
...
!ArrayLength(peer->messages))
        {
                do
                {
                        CHECK_RESULT(gpiSendData(connection, peer->sock,
&string[total], remaining, &closed, &sent, "PT")); if(sent)
                        {
                                total += sent;
                                remaining -= sent;
                        }
...
GPResult gpiSendOrBufferString(GPConnection* connection, GPIPeer_st peer,
                               char* string) {
  return gpiSendOrBufferStringLenToPeer(connection, peer, string,
                                        (int)strlen(string));
}

GPResult gpiSendOrBufferInt(GPConnection* connection, GPIPeer_st peer,
                            int num) {
  char intValue[16];
  sprintf(intValue, "%d", num);
...
GPResult gpiSendOrBufferUInt(GPConnection* connection, GPIPeer_st peer,
                             unsigned int num) {
  char intValue[16];
  sprintf(intValue, "%u", num);
  return gpiSendOrBufferString(connection, peer, intValue);
...
GPResult gpiRecvToBuffer(GPConnection* connection, SOCKET sock,
                         GPIBuffer* inputBuffer, int* bytesRead,
                         GPIBool* connClosed, char id[3]) {
  char* buffer;
  int len;
  int size;
  int rcode;
    ...
    if (gsiSocketIsError(rcode)) {
...
#if defined(GPI_DUMP_NET_TRAFFIC) && defined(GSI_COMMON_DEBUG)
      {
        ...
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network,
                      GSIDebugLevel_RawDump, "RECV%04d(%s): %s\n", recvCount++,
                      id, buf);
        freeclear(buf);
      }
#elif defined(GSI_COMMON_DEBUG)
      {
        static int recvCount;
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network,
                      GSIDebugLevel_RawDump, "RECV%04d(%s): %d\n", recvCount++,
                      id, rcode);
      }
#endif
...
GPResult gpiSendFromBuffer(GPConnection* connection, SOCKET sock,
                           GPIBuffer* outputBuffer, GPIBool* connClosed,
  ...
  do {
    CHECK_RESULT(gpiSendData(connection, sock, &buffer[pos + total], remaining,
                             &closed, &sent, id));
    if (sent) {
      total += sent;
      remaining -= sent;
...
GPResult gpiSendBufferToPeer(GPConnection* connection, unsigned int ip,
  ...
  // length of message remaining must be smaller than total buffer size minus
  // gt2 reliable msg header size minus in order to send the message in one
  // shot.
  if ((int)remaining <= (gsUdpEngineGetPeerOutBufferFreeSpace(ip, port) -
                         GS_UDP_RELIABLE_MSG_HEADER - GS_UDP_MSG_HEADER_LEN)) {
    gsUdpEngineSendMessage(ip, port, iconnection->mHeader, &buffer[pos],
                           remaining, gsi_true);
    total = remaining;
    remaining = 0;
  } else {
    ...
    do {
      ...
      sendAmount =
          freeSpace - (GS_UDP_MSG_HEADER_LEN + GS_UDP_RELIABLE_MSG_HEADER);
      if (sendAmount <= (GS_UDP_MSG_HEADER_LEN + GS_UDP_RELIABLE_MSG_HEADER))
        break;
      if (gsUdpEngineSendMessage(ip, port, iconnection->mHeader,
                                 &buffer[pos + total], sendAmount,
                                 gsi_true) == GS_UDP_SEND_FAILED)
        break;
      total += sendAmount;
  ...
  *message = NULL;
  ...
  if (str != NULL) {
    ...
    *str = '\0';
    ...
    // Read the header.
    ///////////////////
    if (!gpiValueForKey(inputBuffer->buffer, "\\m\\", intValue,
                        sizeof(intValue)))
      return GP_NETWORK_ERROR;
    *type = atoi(intValue);
    ...
    // Get the length.
    //////////////////
    if (!gpiValueForKey(inputBuffer->buffer, "\\len\\", intValue,
                        sizeof(intValue)))
      return GP_NETWORK_ERROR;
    len = atoi(intValue);
    len++;
...
GPResult gpiClipBufferToPosition(GPConnection* connection, GPIBuffer* buffer) {
  ...
  if (buffer->len)
    memmove(buffer->buffer, buffer->buffer + buffer->pos,
