...
GPResult gpiProcessConnectionManager(GPConnection* connection) {
  ...
  do {
    ...
    CHECK_RESULT(gpiSendFromBuffer(connection, iconnection->cmSocket,
                                   &iconnection->outputBuffer, &connClosed,
                                   GPITrue, "CM"));

    // Read everything the connection manager sent.
    ///////////////////////////////////////////////
    result =
        gpiRecvToBuffer(connection, iconnection->cmSocket,
                        &iconnection->socketBuffer, &len, &connClosed, "CM");
    if (result != GP_NO_ERROR) {
    ...
    ////////////////////////////////////////
    while ((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) !=
           NULL) {
      ...
      if (len > iconnection->inputBufferSize) {
        ...
        tempPtr =
            (char*)gsirealloc(iconnection->inputBuffer,
                              (unsigned int)iconnection->inputBufferSize + 1);
        if (tempPtr == NULL)
          Error(connection, GP_MEMORY_ERROR, "Out of memory.");
        iconnection->inputBuffer = tempPtr;
      }
      memcpy(iconnection->inputBuffer, iconnection->socketBuffer.buffer,
             (unsigned int)len + 1);
      ...
      ///////////////////////////////////////////////////////
      iconnection->socketBuffer.len -=
          (next - iconnection->socketBuffer.buffer);
      memmove(iconnection->socketBuffer.buffer, next,
              (unsigned int)iconnection->socketBuffer.len + 1);
      ...
      if (str != NULL) {
        ...
        if (!gpiFindOperationByID(connection, &operation, id)) {
          ...
          /////////////////////////
          CHECK_RESULT(gpiProcessOperation(connection, operation,
      ...
      //////////////////////////////////
        ...
        if (gpiCheckForError(connection, iconnection->inputBuffer, GPITrue)) {
          ...
          CHECK_RESULT(
              gpiProcessRecvBuddyMessage(connection, iconnection->inputBuffer));
          ...
          /////////////////////////
          gpiValueForKey(iconnection->inputBuffer, "\\lt\\",
                         iconnection->loginTicket,
                         sizeof(iconnection->loginTicket));
        } else if (strncmp(iconnection->inputBuffer, "\\bsi\\", 5) == 0) {
          CHECK_RESULT(gpiProcessRecvBuddyStatusInfo(connection,
                                                     iconnection->inputBuffer));
        } else {
    ...
    if (connClosed && iconnection->connectState != GPI_PROFILE_DELETING) {
      ...
      gpiSetError(connection, GP_CONNECTION_CLOSED,
...
GPResult gpiProcess(GPConnection* connection, int blockingOperationID) {
  ...
  */
  ...
  if (iconnection->fatalError) {
    ...
    // assert(!((result != GP_NO_ERROR) && (iconnection->connectState !=
    // GPI_CONNECTED)));
  }
...
void gpiReport(GPConnection* connection, void (*report)(const char* output)) {
  ...
  sprintf(buf, "%d profiles %d bytes (%d avg)", nProfiles, nTotalMemory,
          nTotalMemory / max(nProfiles, 1));
  report(buf);
  if (nProfiles) {
    sprintf(buf, "UserID: %d (%d%%)", nUserID, nUserID * 100 / nProfiles);
    report(buf);
    sprintf(buf, "BuddyStatus: %d (%d%%) %d bytes (%d avg)", nBuddyStatus,
            nBuddyStatus * 100 / nProfiles, nBuddyMemory,
            nBuddyMemory / max(nBuddyStatus, 1));
    report(buf);
    sprintf(buf, "InfoCache: %d (%d%%) %d bytes (%d avg)", nInfoCache,
            nInfoCache * 100 / nProfiles, nInfoMemory,
