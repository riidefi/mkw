...
GPResult gpiFailedOpCallback(GPConnection* connection,
  ...
  if (callback.callback != NULL) {
    ...
    case GPI_NEW_PROFILE: {
      ...
      arg =
          (GPNewProfileResponseArg*)gsimalloc(sizeof(GPNewProfileResponseArg));
      if (arg == NULL)
        Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    case GPI_PROFILE_SEARCH: {
      ...
      arg = (GPProfileSearchResponseArg*)gsimalloc(
          sizeof(GPProfileSearchResponseArg));
      if (arg == NULL)
        Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpiAddOperation(GPConnection* connection, int type, void* data,
                         GPIOperation** op, GPEnum blocking,
...
GPIBool gpiFindOperationByID(const GPConnection* connection,
                             GPIOperation** operation, int id) {
  GPIOperation* op;
  GPIConnection* iconnection = (GPIConnection*)*connection;
...
GPResult gpiProcessOperation(GPConnection* connection, GPIOperation* operation,
                             const char* input) {
  ...
  default:
    gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
                  "gpiProcessOperation was passed an operation with an invalid "
                  "type (%d)\n",
                  operation->type);
    assert(0);
    break;
