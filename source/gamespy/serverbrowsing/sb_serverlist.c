...
// private function used to compare the key values based on a previously defined
// sortkey
...
***/
static int GS_STATIC_CALLBACK IntKeyCompare(const void* entry1,
                                            const void* entry2) {
...
static int GS_STATIC_CALLBACK FloatKeyCompare(const void* entry1,
                                              const void* entry2) {
  SBServer server1 = *(SBServer*)entry1, server2 = *(SBServer*)entry2;
  double f;
  const char* currsortkey = (const char*)g_sortserverlist->currsortinfo.sortkey;
...
static int GS_STATIC_CALLBACK StrCaseKeyCompare(const void* entry1,
                                                const void* entry2) {
  SBServer server1 = *(SBServer*)entry1, server2 = *(SBServer*)entry2;
  int diff;
  const char* currsortkey = (const char*)g_sortserverlist->currsortinfo.sortkey;
...
static int GS_STATIC_CALLBACK StrNoCaseKeyCompare(const void* entry1,
                                                  const void* entry2) {
  SBServer server1 = *(SBServer*)entry1, server2 = *(SBServer*)entry2;
  int diff;
  const char* currsortkey = (const char*)g_sortserverlist->currsortinfo.sortkey;
...
void SBServerListSort(SBServerList* slist, SBBool ascending,
                      SortInfo sortinfo) {
  ...
  if (_tcslen(slist->prevsortinfo.sortkey) ==
      0) // set prev to current if not initialized
    slist->prevsortinfo = sortinfo;
  else if (strcmp((const char*)sortinfo.sortkey,
                  (const char*)slist->currsortinfo.sortkey) != 0)
    slist->prevsortinfo =
        slist->currsortinfo; // only set new sort if different than prev
...
int SBServerListFindServerByIP(SBServerList* slist, goa_uint32 ip,
                               unsigned short port) {
  int numservers;
  SBServer server;
  int i;
  ...
  for (i = 0; i < numservers; i++) {
    ...
    if (SBServerGetPublicInetAddress(server) == ip &&
        SBServerGetPublicQueryPortNBO(server) == port) {
      return i;
    }
  }
  return -1;
...
void SBServerListClear(SBServerList* slist) {
  // we need to add each server to the dead list so it can be freed after the
  // clear
  int i;
...
void SBServerListInit(SBServerList* slist, const char* queryForGamename,
                      const char* queryFromGamename, const char* queryFromKey,
                      int queryFromVersion, SBBool lanBrowse,
                      SBListCallBackFn callback, void* instance) {
  assert(slist != NULL);
...
static void ErrorDisconnect(SBServerList* slist) {
  ...
  if ((slist->inbufferlen > 0) &&
      ((unsigned int)slist->inbufferlen > strlen(QUERY_ERROR)) &&
...
static SBError ServerListConnect(SBServerList* slist) {
  ...
  if (SBOverrideMasterServer != NULL)
    ...
    sprintf(masterHostname, "%s.ms%d." GSI_DOMAIN_NAME, slist->queryforgamename,
            masterIndex);
  ...
  if (saddr.sin_addr.s_addr == INADDR_NONE) {
    ...
    if (!hent)
      return sbe_dnserror;
    memcpy(&saddr.sin_addr.s_addr, hent->h_addr_list[0],
...
#define CALCULATEODDMODE(buffer, i, oddmode)                                   \
  ((buffer[i - 1] & 1) ^ (i & 1) ^ oddmode ^ (buffer[0] & 1) ^                 \
   ((buffer[0] < 79) ? 1 : 0) ^ ((buffer[i - 1] < buffer[0]) ? 1 : 0));
static void SetupListChallenge(SBServerList* slist) {
  int i;
  int oddmode;

  slist->mychallenge[0] =
      (char)(33 + rand() % 93); // use chars in the range 33 - 125
  oddmode = 0;
  for (i = 1; i < LIST_CHALLENGE_LEN; i++) {
    ...
    if ((oddmode && (slist->mychallenge[i] & 1) == 0) ||
        (!oddmode && ((slist->mychallenge[i] & 1) == 1)))
      slist->mychallenge[i]++;
  }
}
...
SBError SBServerListConnectAndQuery(SBServerList* slist, const char* fieldList,
                                    const char* serverFilter, int options,
                                    int maxServers) {
  SBError err;
  int ret;
  int requestLen;
  ...
  BufferAddData(&requestBuf, slist->mychallenge, LIST_CHALLENGE_LEN,
                &requestLen);
...
SBError SBServerListGetLANList(SBServerList* slist, unsigned short startport,
                               unsigned short endport, int queryversion) {
  struct sockaddr_in saddr;
  unsigned short i;
  unsigned char qr2_echo_request[] = {QR2_MAGIC_1, QR2_MAGIC_2, 0x02, 0x00,
                                      0x00,        0x00,        0x00, 0x00};
...
// enable broadcasting where needed
  ...
  {
    int optval = 1;
    if (setsockopt(slist->slsocket, SOL_SOCKET, SO_BROADCAST, (char*)&optval,
                   sizeof(optval)) != 0)
      return sbe_socketerror;
  }
#endif
  ...
  // Added for multihomed machines.  Allows the user to specify which adapter to
  // use
  if (slist->mLanAdapterOverride != NULL) {
    ...
    if (0 != bind(slist->slsocket, (struct sockaddr*)&aClassCAddr,
                  sizeof(aClassCAddr)))
      return sbe_socketerror;
  }
  ...
  for (i = startport; i <= endport; i += 1) {
    ...
    if (queryversion == QVERSION_QR2) // send a QR2 echo request
      sendto(slist->slsocket, (char*)qr2_echo_request, qr2requestlen, 0,
             (struct sockaddr*)&saddr, sizeof(saddr));
    else // send a GOA echo request
      sendto(slist->slsocket, "\\echo\\test", 10, 0, (struct sockaddr*)&saddr,
             sizeof(saddr));
  }
...
static void InitCryptKey(SBServerList* slist, char* key, int keylen) {
  // combine our secret key, our challenge, and the server's challenge into a
  // crypt key
  int i;
  ...
  GOACryptInit(&(slist->cryptkey), (unsigned char*)(slist->mychallenge),
               LIST_CHALLENGE_LEN);
}
...
static void ParseServerIPPort(SBServerList* slist, char* buf, int len,
                              goa_uint32* ip, unsigned short* port) {
  unsigned char flags;
  if (len < 5)
...
static int ParseServer(SBServerList* slist, SBServer server, char* buf, int len,
                       int usepopularlist) {
  int numkeys;
  int i;
  short sval;
  ...
  if (flags & HAS_KEYS_FLAG) // parse the keys
  {
    ...
    for (i = 0; i < numkeys; i++) {
      ...
      case KEYTYPE_SHORT:
        memcpy(&sval, buf, 2);
        SBServerAddIntKeyValue(server, ki->keyName,
                               ntohs((unsigned short)sval));
        buf += 2;
        len -= 2;
        break;
      case KEYTYPE_STRING:
        ...
        {
          SBServerAddKeyValue(server, ki->keyName,
                              slist->popularvalues[strindex]);
        }
        break;
      }
    }
    SBServerSetState(
        server, (unsigned char)(SBServerGetState(server) | STATE_BASICKEYS));
  }
  if (flags & HAS_FULL_RULES_FLAG) // got the full rules in there
  {
    ...
    SBServerSetState(
        server, (unsigned char)(SBServerGetState(server) | STATE_FULLKEYS));
  }
...
Standard query port - 2 bytes - or 0xFFFF if the master does not know about this
gamename yet [rest only follows if they requested a server slist]
--key slist
...
static SBError ProcessMainListData(SBServerList* slist) {
  ...
  case pi_fixedheader:
    ...
    slist->ListCallback(slist, slc_publicipdetermined, SBNullServer,
                        slist->instance);
    memcpy(&slist->defaultport, inbuf + 4, 2);
    if (slist->defaultport ==
        0xFFFF) // there was an error-  grab the error string if present..
    {
      if (NTSLengthSB(inbuf + FIXED_HEADER_LEN, inlen - FIXED_HEADER_LEN) ==
          -1) // not all there
        break;
    ...
    if ((slist->queryoptions & NO_SERVER_LIST) ||
        slist->defaultport == 0xFFFF) {
      slist->pstate = pi_finished;
      slist->state = sl_connected;
      break; // no more data expected (if we didn't request any more, or the
             // server doesn't know about this game)
    }
    ...
    // and fall through
  case pi_keylist:
    if (slist->expectedelements ==
        -1) // we haven't read the initial count of keys yet..
    {
      if (inlen < 1)
        break;
  ...
  case pi_uniquevaluelist:
    if (slist->expectedelements ==
        -1) // we haven't read the # of unique values yet..
    {
      if (inlen < 1)
        break;
      slist->expectedelements = (unsigned char)(inbuf[0]);
  ...
  case pi_servers:
    ...
    do {
      ...
      else if (reqlen == -1) // that was the last server!
      {
        ...
        slist->pstate = pi_finished;
        slist->state = sl_connected;
        slist->ListCallback(slist, slc_initiallistcomplete, SBNullServer,
                            slist->instance);
        break;
      }
      inbuf += reqlen;
...
static SBError ProcessPlayerSearch(SBServerList* slist, char* buf, int len) {
  ...
  for (i = 0; i < resultCount; i++) {
    ...
    slist->PlayerSearchCallback(slist, nick, ip, port, lastSeen, buf,
                                slist->instance);
...
static SBError ProcessMaploop(SBServerList* slist, char* buf, int len) {
  ...
  if (slist->MaploopCallback == NULL)
    return sbe_noerror;
  slist->MaploopCallback(slist, server, changeTime, i, mapList,
                         slist->instance);
  return sbe_noerror;
}
...
static SBError ProcessAdHocData(SBServerList* slist) {
  ...
  while (slist->inbufferlen >=
         3) // while there is at least 1 message in there.. (2 bytes for the
            // size + 1 for the type)
  {
    ...
    if (slist->inbufferlen != 0 &&
        slist->inbuffer != NULL) // if anything is left, shift it over..
    {
      memmove(slist->inbuffer, slist->inbuffer + msglen,
              (size_t)slist->inbufferlen);
    }
    if (ret != sbe_noerror)
      break;
  }
...
static SBError ProcessIncomingData(SBServerList* slist) {
  ...
  len = recv(slist->slsocket, slist->inbuffer + slist->inbufferlen,
             INCOMING_BUFFER_SIZE - slist->inbufferlen, 0);
  ...
  err = sbe_noerror;
  if (slist->state == sl_connected ||
      slist->pstate > pi_cryptheader) // decrypt any new data..
  {
    GOADecrypt(&(slist->cryptkey), (unsigned char*)(slist->inbuffer + oldlen),
               slist->inbufferlen - oldlen);
  }
  ...
  if (err != sbe_noerror)
    return err;
  // always need to check this after mainlistdata, in case some extra data has
  // some in (e.g. key list for push)
  if (slist->state == sl_connected && slist->inbufferlen > 0)
    return ProcessAdHocData(slist);
  return sbe_noerror;
}

SBError SBGetServerRulesFromMaster(SBServerList* slist, goa_uint32 ip,
                                   unsigned short port) {
...
SBError SBSendMessageToServer(SBServerList* slist, goa_uint32 ip,
                              unsigned short port, const char* data, int len) {
  char requestBuffer[16];
  unsigned short tmp;
  unsigned short msgLen = 9;
  SBError ret;
...
SBError SBSendPlayerSearchRequest(SBServerList* slist, char* searchName,
                                  int searchOptions, int maxResults,
...
SBError SBSendMaploopRequest(SBServerList* slist, SBServer server,
...
SBError SBSendNatNegotiateCookieToServer(SBServerList* slist, goa_uint32 ip,
                                         unsigned short port, int cookie) {
  ...
  return SBSendMessageToServer(slist, ip, port, (char*)negotiateBuffer,
                               NATNEG_MAGIC_LEN + 4);
}

static SBError ProcessLanData(SBServerList* slist) {
  __attribute__((aligned(32))) char
      indata[1500]; // make sure we have enough room for a large UDP packet
  struct sockaddr_in saddr;
  ...
  while (CanReceiveOnSocket(slist->slsocket)) // we break if the select fails
  {
    ...
    foundexisting = SBServerListFindServerByIP(slist, saddr.sin_addr.s_addr,
                                               saddr.sin_port);
  ...
  if (current_time() - slist->lanstarttime >
      SL_LAN_SEARCH_TIME) // done waiting for replies
  {
    closesocket(slist->slsocket);
    slist->slsocket = INVALID_SOCKET;
    slist->state = sl_disconnected;
