...
qr2_error_t qr2_init_socketW(/*[out]*/ qr2_t* qrec, SOCKET s, int boundport,
...
qr2_error_t qr2_create_socket(/*[out]*/ SOCKET* sock, const char* ip,
                              /*[in/out]*/ int* port);
...
qr2_error_t qr2_init_socketA(/*[out]*/ qr2_t* qrec, SOCKET s, int boundport,
  ...
  if (ispublic) {
    int override_ = qr2_hostname[0];
    if (!override_)
      sprintf(hostname, "%s.master." GSI_DOMAIN_NAME, gamename);
    ret = get_sockaddrin(override_ ? qr2_hostname : hostname, MASTER_PORT,
                         &(cr->hbaddr), NULL);

    if (ret == 1) {
      gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Network,
                    GSIDebugLevel_Comment, "%s resolved to %s\r\n",
                    override_ ? qr2_hostname : hostname,
                    inet_ntoa(cr->hbaddr.sin_addr));
    } else {
      gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Network,
                    GSIDebugLevel_HotError, "Failed on DNS lookup for %s \r\n",
                    override_ ? qr2_hostname : hostname);
    }
...
qr2_error_t qr2_init_socketW(/*[out]*/ qr2_t* qrec, SOCKET s, int boundport,
  ...
  return qr2_init_socketA(qrec, s, boundport, gamename_A, secretkey_A, ispublic,
                          natnegotiate, server_key_callback,
                          player_key_callback, team_key_callback,
                          key_list_callback, playerteam_count_callback,
...
qr2_error_t qr2_create_socket(/*[out]*/ SOCKET* sock, const char* ip,
  ...
  while (baseport < maxport) {
    ...
    if (saddr.sin_addr.s_addr ==
        htonl(0x7F000001)) // localhost -- we don't want that!
...
qr2_error_t qr2_initA(/*[out]*/ qr2_t* qrec, const char* ip, int baseport,
  ...
  ret =
      qr2_init_socketA(qrec, hbsock, baseport, gamename, secret_key, ispublic,
                       natnegotiate, server_key_callback, player_key_callback,
                       team_key_callback, key_list_callback,
...
qr2_error_t qr2_initW(/*[out]*/ qr2_t* qrec, const unsigned short* ip,
  ...
  return qr2_initA(qrec, (ip != NULL) ? ip_A : NULL, baseport, gamename_A,
                   secretkey_A, ispublic, natnegotiate, server_key_callback,
                   player_key_callback, team_key_callback, key_list_callback,
                   playerteam_count_callback, adderror_callback, userdata);
}
...
void qr2_define_some_strings() {
  ...
  foo = (void (*)(void))x;
  foo();
  x = "CanReceiveOnSocket() returned true, but recvfrom return 0!\r\n"; // 0x8027d0c8
  foo = (void (*)(void))x;
  foo();
  x = "CanReceiveOnSocket() returned true, but recvfrom failed!\r\n"; // 0x8027d108
  foo = (void (*)(void))x;
  foo();
}
...
void qr2_check_send_heartbeat(qr2_t qrec) {
  ...
  if (qrec->listed_state > 0 &&
      tc - qrec->lastheartbeat >
          FIRST_HB_TIME) { // check to see if we haven't gotten a query yet
  ...
  } else if (qrec->userstatechangerequested &&
             (tc - qrec->lastheartbeat > MIN_STATECHANGED_HB_TIME))
    send_heartbeat(qrec, 1); // Send out pending statechange request
  else if (tc - qrec->lastheartbeat > HB_TIME || qrec->lastheartbeat == 0 ||
           tc < qrec->lastheartbeat)
    send_heartbeat(qrec, 0); // Send out a normal hearbeat

  if (current_time() - qrec->lastka >
      KA_TIME) // send a keep alive (to keep NAT port mappings the same if
               // possible)
    send_keepalive(qrec);
}
...
void qr2_send_statechanged(qr2_t qrec) {
  ...
  qrec->userstatechangerequested =
      0; // clear the flag in case a queued statechange was still pending
}
...
void qr2_shutdown(qr2_t qrec) {
  ...
  if (INVALID_SOCKET != qrec->hbsock &&
      qrec->read_socket) // if we own the socket
  {
  ...
  if (qrec !=
      &static_qr2_rec) // need to gsifree it, it was dynamically allocated
  {
    gsifree(qrec);
  }
...
gsi_bool qr2_buffer_addA(qr2_buffer_t outbuf, const char* value) {
  ...
  GS_ASSERT(value) {
    ...
    if (copylen > AVAILABLE_BUFFER_LEN(outbuf))
      copylen =
          AVAILABLE_BUFFER_LEN(outbuf); // max length we can fit in the buffer
    if (copylen <= 0)
...
static void enum_local_ips() {
  ...
  for (num_local_ips = 0; num_local_ips < MAX_LOCAL_IP; num_local_ips++) {
    if (phost->h_addr_list[num_local_ips] == 0)
      break;
    memcpy(&local_ip_list[num_local_ips], phost->h_addr_list[num_local_ips],
...
static void gs_encrypt(uchar* key, int key_len, uchar* buffer_ptr,
                       int buffer_len) {
  short counter;
  uchar x, y, xorIndex;
  uchar state[256];
...
static void compute_challenge_response(qr2_t qrec, qr2_buffer_t buf,
                                       char* challenge, int challengelen) {
  ...
  gs_encrypt((uchar*)qrec->secret_key, (int)strlen(qrec->secret_key),
             (uchar*)encrypted_val, challengelen - 1);
  gs_encode((uchar*)encrypted_val, challengelen - 1,
            (uchar*)(buf->buffer + buf->len));
...
static void qr_build_partial_query_reply(qr2_t qrec, qr2_buffer_t buf,
                                         qr2_key_type keytype, int keycount,
                                         uchar* keys) {
  ...
  if (keytype == key_player ||
      keytype == key_team) // need to add the player/team counts
  {
...
static void qr_build_query_reply(qr2_t qrec, qr2_buffer_t buf,
                                 int serverkeycount, uchar* serverkeys,
                                 int playerkeycount, uchar* playerkeys,
                                 int teamkeycount, uchar* teamkeys) {
  qr_build_partial_query_reply(qrec, buf, key_server, serverkeycount,
                               serverkeys);
  qr_build_partial_query_reply(qrec, buf, key_player, playerkeycount,
                               playerkeys);
...
static gsi_bool
qr_build_split_query_reply(qr2_t qrec, qr2_buffer_t buf,
                           struct QRSplitQueryProgress* progress) {
  unsigned char* packetNumPos =
      NULL; // Used to store the byte position of the packet number
  ...
  *packetNumPos = (gsi_u8)progress->mCurPacketNum++;
  ...
  while (progress->mCurKeyType < key_type_count) {
    ...
    if (progress->mKeyBuffer.numkeys == 0)
      qrec->key_list_callback(progress->mCurKeyType, &progress->mKeyBuffer,
                              qrec->udata);

    // Get the list of players/teams if we don't have it already
    if (progress->mCurSubCount == 0 && progress->mCurKeyType != key_server)
      progress->mCurSubCount =
          qrec->playerteam_count_callback(progress->mCurKeyType, qrec->udata);
    ...
    while (progress->mCurKeyIndex < progress->mKeyBuffer.numkeys) {
      // check buffer space
      int aRegisteredKeyIndex =
          progress->mKeyBuffer.keys[progress->mCurKeyIndex];
      ...
      if (progress->mCurKeyType == key_server) {
        ...
        while (progress->mCurSubIndex < progress->mCurSubCount) {
          ...
          if (progress->mCurKeyType == key_player)
            qrec->player_key_callback(aRegisteredKeyIndex,
                                      progress->mCurSubIndex, buf, qrec->udata);
          else if (progress->mCurKeyType == key_team)
            qrec->team_key_callback(aRegisteredKeyIndex, progress->mCurSubIndex,
                                    buf, qrec->udata);
...
static void qr_process_query(qr2_t qrec, qr2_buffer_t buf, uchar* qdata,
                             int len, struct sockaddr* sender) {
  ...
  if ((exflags & QR2_EXFLAG_SPLIT) == QR2_EXFLAG_SPLIT) {
    ...
    qr_build_query_reply(qrec, buf, serverkeycount, serverkeys, playerkeycount,
                         playerkeys, teamkeycount, teamkeys);
    sendto(qrec->hbsock, buf->buffer, buf->len, 0, sender,
           sizeof(struct sockaddr_in));
  }
...
/*
static void qr_build_partial_old_query_reply(qr2_t qrec, qr2_buffer_t buf,
qr2_key_type keytype)
{
        ...
        if (keytype == key_player || keytype == key_team) //need to add the
player/team counts
        {
                playerteamcount = qrec->playerteam_count_callback(keytype,
qrec->udata); } else playerteamcount = 1;
        ...
        for (i = 0 ; i < kb.numkeys ; i++)
        {
                ...
                } else //need to look it up for each player/team
                {
                        ...
                        for (pindex = 0 ; pindex < playerteamcount ; pindex++)
                        {
                                ...
                                len = buf->len;
                                if (keytype == key_player)
                                        qrec->player_key_callback(kb.keys[i],
pindex, buf, qrec->udata); else if (keytype == key_team)
                                        qrec->team_key_callback(kb.keys[i],
pindex, buf, qrec->udata); if(len == buf->len) qr2_buffer_addA(buf, "");
                                buf->buffer[buf->len - 1] = '\\';
                        }
                }
        }
...
static int qr_got_recent_message(qr2_t qrec, int msgkey) {
  ...
  qrec->cur_message_key =
      (qrec->cur_message_key + 1) % RECENT_CLIENT_MESSAGES_TO_TRACK;
  qrec->client_message_keys[qrec->cur_message_key] = msgkey;
  return 0;
}

// Send a random value to the user, to verify IP address
static gsi_bool qr2_process_ip_verify(qr2_t qrec, struct qr2_buffer_s* buf,
...
static gsi_bool qr2_check_ip_verify(qr2_t qrec, struct sockaddr_in* sender,
                                    gsi_u32 ipverify) {
  int i = 0;
  for (; i < QR2_IPVERIFY_ARRAY_SIZE; i++) {
    if (qrec->ipverify[i].addr.sin_addr.s_addr == sender->sin_addr.s_addr &&
...
void qr2_parse_queryA(qr2_t qrec, char* query, int len,
  ...
  case PACKET_PREQUERY_IP_VERIFY:
    ...
    if (gsi_is_true(
            qr2_process_ip_verify(qrec, &buf, (struct sockaddr_in*)sender)))
  ...
  case PACKET_QUERY:
    ...
    // When using query challenge option, verify that the client sent a
    // PREQUERY_IP_VERIFY
    if ((qrec->backendoptions & QR2_OPTION_USE_QUERY_CHALLENGE) ==
        QR2_OPTION_USE_QUERY_CHALLENGE) {
      if (len < 4)
      ...
      if (gsi_is_false(qr2_check_ip_verify(qrec, (struct sockaddr_in*)sender,
                                           ipverify))) {
  ...
  case PACKET_ADDERROR:
    gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Network,
                  GSIDebugLevel_WarmError, "Processing adderror packet\r\n");
  ...
  case PACKET_KEEPALIVE:
    ...
    return; // if we get a keep alive, ignore it and return (just used to tell
            // us the server knows about us)
  default:
...
static void send_keepalive(qr2_t qrec) {
  ...
  sendto(qrec->hbsock, buf.buffer, buf.len, 0,
         (struct sockaddr*)&(qrec->hbaddr), sizeof(struct sockaddr_in));
...
static void send_heartbeat(qr2_t qrec, int statechanged) {
  ...
  // ret = (int)sendto(qrec->hbsock, buf.buffer, buf.len, 0, (struct sockaddr
  // *)&(qrec->hbaddr), sizeof(struct sockaddr_in));
  sendto(qrec->hbsock, buf.buffer, buf.len, 0,
         (struct sockaddr*)&(qrec->hbaddr), sizeof(struct sockaddr_in));
