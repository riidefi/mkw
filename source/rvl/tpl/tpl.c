  ...
  for (u16 i = 0; i < pal->num_descriptors; i++) {
    ...
    if (pal->descriptors[i].clut_header) {
      pal->descriptors[i].clut_header =
          (TPLClutHeader*)((u32)(pal->descriptors[i].clut_header) +
                           (u32)pal);
...
void TPLGetGXTexObjFromPalette(TPLPalette* pal, GXTexObj* to, u32 id) {
  ...
  GXInitTexObj(to, desc->header->data, desc->header->width,
               desc->header->height, desc->header->format,
               desc->header->wrap_s, desc->header->wrap_t, mipMap);
  GXInitTexObjLOD(to, desc->header->filter_min,
                  desc->header->filter_mag, desc->header->lod_min,
                  desc->header->lod_max, desc->header->lod_bias, 0,
                  desc->header->edge_lod_enabled, 0);
}
