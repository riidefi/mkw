...
{
	DWCiAssert(*(u32*)((u32)block - 32) == DWCi_MEMFUNC_SIGNATURE, "dwc_memfunc.c", 56, "Failed assertion header->signature == DWCi_MEMFUNC_SIGNATURE");
...
void* DWC_AllocEx(DWCAllocType type, u32 size, int align)
{
	...
	DWCiAssert(DWCi_Np_GetInitFlag(), "dwc_memfunc.c", 158, "DWC hasn't initialized yet.");

	DWCiAssert(s_alloc, "dwc_memfunc.c", 160, "DWC_MEMFUNC: Allocator function isn't defined.\n");
	DWCiAssert(align <= 32 && align >= -32, "dwc_memfunc.c", 161, "DWC_Alloc() dosen't support to align over 32 bytes order");
...
{
	DWCiAssert(DWCi_Np_GetInitFlag(), "dwc_memfunc.c", 191, "DWC hasn't initialized yet.");
