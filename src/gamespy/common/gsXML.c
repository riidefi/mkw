...
const char*
    GS_XML_SOAP_DEFAULT_NAMESPACES[GS_XML_SOAP_DEFAULT_NAMESPACE_COUNT] = {
...
static gsi_bool gsiXmlUtilSkipWhiteSpace(GSIXmlStreamReader* stream,
                                         const char* buffer, int len, int* pos);
static gsi_bool gsiXmlUtilParseName(GSIXmlStreamReader* stream,
                                    const char* buffer, int len, int* pos,
                                    GSIXmlString* strOut);
...
GSXmlStreamWriter gsXmlCreateStreamWriter(const char** namespaces, int count) {
  ...
  GS_ASSERT((namespaces == NULL && count == 0) ||
            (namespaces != NULL && count != 0));
  ...
  for (i = 0; i < GS_XML_SOAP_DEFAULT_NAMESPACE_COUNT; i++) {
    if (gsi_is_false(gsiXmlUtilWriteChar(newStream, ' ')) ||
        gsi_is_false(
            gsiXmlUtilWriteString(newStream, GS_XML_SOAP_NAMESPACE_PREFIX)) ||
        gsi_is_false(gsiXmlUtilWriteString(
            newStream, GS_XML_SOAP_DEFAULT_NAMESPACES[i]))) {
      gsifree(newStream->mBuffer);
      gsifree(newStream);
      return NULL; // OOM
    }
...
GSXmlStreamReader gsXmlCreateStreamReader() {
  ...
  newStream->mElementArray =
      ArrayNew(sizeof(GSIXmlElement), GS_XML_INITIAL_ELEMENT_ARRAY_COUNT,
               gsiXmlUtilElementFree);
  ...
  newStream->mAttributeArray =
      ArrayNew(sizeof(GSIXmlAttribute), GS_XML_INITIAL_ATTRIBUTE_ARRAY_COUNT,
               gsiXmlUtilAttributeFree);
...
static gsi_bool gsiXmlUtilParseElement(GSIXmlStreamReader* stream, char* buffer,
  ...
  while (*pos < len && isalnum(buffer[*pos])) {
    ...
    (*pos)++; // skip the '='
    GS_XML_CHECK(
        gsiXmlUtilParseString(stream, buffer, len, pos, &newAttr.mValue));
    GS_XML_CHECK(gsiXmlUtilSkipWhiteSpace(stream, buffer, len, pos));
  ...
  if ((!isalnum(buffer[startPos + 1]) &&
       buffer[*pos] == buffer[startPos + 1]) ||
      buffer[*pos] == '/') {
    (*pos)++;
    GS_XML_CHECK(gsiXmlUtilSkipWhiteSpace(stream, buffer, len, pos));
    if (buffer[*pos] != '>')
  ...
  if (buffer[*pos] != '<') {
    GS_XML_CHECK(
        gsiXmlUtilParseValue(stream, buffer, len, pos, &newElem.mValue));
  ...
  while (*pos < len) {
    ...
    if (buffer[*pos] == '/') {
      ...
      if (0 != strncmp((const char*)newElem.mName.mData, &buffer[*pos],
                       (size_t)newElem.mName.mLen))
      ...
      *pos = childStartPos;
      GS_XML_CHECK(
          gsiXmlUtilParseElement(stream, buffer, len, pos, newElem.mIndex));
      gsiXmlUtilSkipWhiteSpace(stream, buffer, len, pos);
    }
  }
  return gsi_false; // EOF before tag close
...
///////////////////////////////////////////////////////////////////////////////
// Parse element name
//   Must begin with a letter and contains only alphanumeric | '_' | '-'
//   characters Element names may consist of two "names",
...
static gsi_bool gsiXmlUtilParseName(GSIXmlStreamReader* stream,
  ...
  while (*pos < len && NULL == strchr(GS_XML_WHITESPACE, buffer[*pos])) {
    // only alpha numeric and '_' characters are allowed, plus one namespace
    // separator ':'
    if (buffer[*pos] == ':') {
      if (gsi_is_true(haveNamespace))
        return gsi_false; // already have a namespace!
      haveNamespace = gsi_true;
    } else if ((buffer[*pos] != '_') && (buffer[*pos] != '-') &&
               (!isalnum(buffer[*pos])))
...
static gsi_bool gsiXmlUtilDecodeString(char* buffer, int* len) {
  ...
  while (readPos < *len) {
    ...
    if (strncmp(&buffer[readPos], "&amp;", 5) == 0) {
      ...
      for (i = 0; i < 4; i++) {
        ch = (char)((digitValue & mask) >>
                    ((3 - i) * 8)); // make 0x00006500 into 0x65
      ...
      // scan digits into memory, do this as a block so that &#357 = 0165h = 01
      // 65
    ...
    memmove(&buffer[readPos], &buffer[readPos + charsToRemove],
            (size_t)(*len - (readPos + charsToRemove)));
...
gsi_bool gsXmlWriteBase64BinaryElement(GSXmlStreamWriter stream,
  ...
  B64InitEncodeStream(&streamData, (const char*)data, len,
...
gsi_bool gsXmlWriteDateTimeElement(GSXmlStreamWriter stream,
  ...
  sprintf(timeString, "%d-%02d-%02dT%02d:%02d:%02dZ", timePtr->tm_year + 1900,
  ...
  for (; readPos >= 0; readPos--) {
    if (gsi_is_true(first)) {
      for (i = 0; i < GS_LARGEINT_DIGIT_SIZE_BYTES; i++) {
        temp = ((lint->mData[readPos] >>
                 (8 * (GS_LARGEINT_DIGIT_SIZE_BYTES - i - 1))) &
                0xFF);
        if (temp != 0)
          break;
      }
...
gsi_bool gsXmlMoveToParent(GSXmlStreamReader stream) {
  ...
  if (reader->mElemReadIndex >= ArrayLength(reader->mElementArray) ||
    ...
    GSIXmlElement* elem =
        (GSIXmlElement*)ArrayNth(reader->mElementArray, reader->mElemReadIndex);
...
gsi_bool gsXmlReadChildAsStringNT(GSXmlStreamReader stream,
  ...
  if (gsi_is_false(
          gsXmlReadChildAsString(stream, matchtag, &strValue, &strLen))) {
    valueOut[0] = '\0';
    return gsi_false;
  } else {
...
gsi_bool gsXmlReadChildAsUnicodeStringNT(GSXmlStreamReader stream,
  ...
  if (gsi_is_false(
          gsXmlReadChildAsString(stream, matchtag, &utf8Value, &utf8Len))) {
    valueOut[0] = '\0';
    return gsi_false;
  } else {
    // Convert into destination buffer
    unicodeLen =
        UTF8ToUCS2StringLen(utf8Value, (unsigned short*)valueOut, utf8Len);
  ...
  for (i = (reader->mValueReadIndex + 1);
    ...
    if (searchValueElem->mParentIndex == reader->mElemReadIndex) {
      ...
      if (gsi_is_true(
        ...
        if (searchValueElem->mValue.mLen == 0 ||
            searchValueElem->mValue.mData == NULL) {
          valueOut[0] = 0;
          *lenOut = 0;
          return gsi_true;
        }
        ...
        if (valueOut == NULL) {
          *lenOut = searchValueElem->mValue.mLen / 2;
          ...
          // note: read position left at this elemtent so next read can record
          // the data
          return gsi_true;
        }
      ...
      if (gsi_is_true(
        ...
        if (valueOut) {
          ...
          if (searchValueElem->mValue.mData)
            B64Decode((char*)searchValueElem->mValue.mData, (char*)valueOut,
                      searchValueElem->mValue.mLen, lenOut,
                      GS_XML_BASE64_ENCODING_TYPE);
          else
            *lenOut = 0;
        } else {
          if (searchValueElem->mValue.mData)
            *lenOut = B64DecodeLen((const char*)searchValueElem->mValue.mData,
                                   GS_XML_BASE64_ENCODING_TYPE);
          else
            *lenOut = 0;
        }
    ...
    if (searchValueElem->mParentIndex == reader->mElemReadIndex) {
      ...
      if (gsi_is_true(
        ...
        if (searchValueElem->mValue.mData == NULL)
          return gsi_false; // invalid type!
        *valueOut =
            gsiStringToInt64((const char*)searchValueElem->mValue.mData);
        return gsi_true;
      }
    }
    // bail if we've reached a higher branch
    ...
    if (searchValueElem->mParentIndex == reader->mElemReadIndex) {
      ...
      if (gsi_is_true(
        ...
        sscanf((const char*)searchValueElem->mValue.mData,
               "%i-%02d-%02dT%02d:%02d:%02d", &timePtr.tm_year, &timePtr.tm_mon,
...
gsi_bool gsiXmlUtilTagMatches(const char* matchtag, GSIXmlString* xmlstr) {
  ...
  while (xmlNamespacePos < xmlstr->mLen &&
         xmlstr->mData[xmlNamespacePos] != ':')
    xmlNamespacePos++;
  ...
  // compare strings
  if (0 == strncmp(matchNoNamespace, (const char*)xmlNoNamespace.mData,
                   (size_t)xmlNoNamespace.mLen))
    return gsi_true;
  else
    return gsi_false;
}
...
gsi_bool gsXmlReadChildAsLargeInt(GSXmlStreamReader stream,
  ...
  // parse the hexbinary
  if (gsi_is_false(
          gsXmlReadChildAsHexBinary(stream, matchtag, (gsi_u8*)valueOut->mData,
...
int gsXmlCountChildren(GSXmlStreamReader stream, const char* matchtag) {
  ...
  for (i = (reader->mElemReadIndex + 1); i < ArrayLength(reader->mElementArray);
    ...
    if (searchElem->mParentIndex == reader->mElemReadIndex) {
      // check match
      if ((matchtag == NULL) ||
          gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchElem->mName))) {
        count++;
      }
    }
    // check if we've reached a higher branch
