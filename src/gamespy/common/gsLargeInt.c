...
static gsi_bool gsiLargeIntPrint(FILE* logFile, const l_word* data,
                                 l_word length);
...
static gsi_bool gsiLargeIntSizePower2(const gsLargeInt_t* src1,
                                      const gsLargeInt_t* src2, l_word* lenout);
static gsi_i32 gsiLargeIntCompare(const l_word* data1, l_word len1,
                                  const l_word* data2, l_word len2);

static gsi_bool gsiLargeIntKMult(const l_word* data1, const l_word* data2,
                                 l_word length, l_word* dest, l_word* lenout,
                                 l_word maxlen);
static gsi_bool gsiLargeIntMult(const l_word* data1, l_word length1,
                                const l_word* data2, l_word length2,
                                l_word* dest, l_word* lenout, l_word maxlen);
static gsi_bool gsiLargeIntDiv(const l_word* src1, l_word length1,
                               const gsLargeInt_t* divisor, gsLargeInt_t* dest,
                               gsLargeInt_t* remainder);

// Dest may be data1 or data2 to support in-place arithmetic
static gsi_bool gsiLargeIntAdd(const l_word* data1, l_word length1,
                               const l_word* data2, l_word length2,
                               l_word* dest, l_word* lenout, l_word maxlen);
static gsi_bool gsiLargeIntSub(const l_word* amount, l_word length1,
                               const l_word* from, l_word length2, l_word* dest,
                               l_word* lenout);

// Special division, removes divisor directly from src1, leaving remainder
static gsi_bool gsiLargeIntSubDivide(l_word* src1, l_word length,
                                     const l_word* divisor, l_word dlen,
                                     gsi_u32 highbit, l_word* quotient);

// Montgomery utilities
// gsi_bool gsiLargeIntSquareM(const gsLargeInt_t *src, const gsLargeInt_t *mod,
// gsi_u32 modPrime, gsi_u32 R, gsLargeInt_t *dest); gsi_bool
// gsiLargeIntMultM(gsLargeInt_t *src1, gsLargeInt_t *src2, const gsLargeInt_t
// *mod, gsi_u32 modPrime, gsLargeInt_t *dest);
gsi_bool gsiLargeIntMultM(gsLargeInt_t* src1, gsLargeInt_t* src2,
                          const gsLargeInt_t* mod, gsi_u32 modPrime,
                          gsLargeInt_t* dest);
...
gsi_bool gsiLargeIntResize(gsLargeInt_t* lint, l_word length) {
  ...
  // otherwise, add zeroes until length is reached
    ...
    memset(&lint->mData[lint->mLength], 0,
           (length - lint->mLength) * sizeof(l_word));
    lint->mLength = length;
    return gsi_true;
  }
}
...
gsi_bool gsiLargeIntSizePower2(const gsLargeInt_t* src1,
                               const gsLargeInt_t* src2, l_word* lenout) {
  unsigned int i = 0;
...
gsi_bool gsLargeIntAdd(const gsLargeInt_t* src1, const gsLargeInt_t* src2,
                       gsLargeInt_t* dest) {
  gsi_bool result =
      gsiLargeIntAdd(src1->mData, src1->mLength, src2->mData, src2->mLength,
                     dest->mData, &dest->mLength, GS_LARGEINT_MAX_DIGITS);
...
static gsi_bool gsiLargeIntAdd(const l_word* data1, l_word length1,
  ...
  //    do not perform length check here, so that we can support oversized
  //    buffers
...
//   strips leading zeroes (gsiLargeIntSub doesn't strip for compatability with
//   karatsuba fixed size numbers)
gsi_bool gsLargeIntSub(const gsLargeInt_t* src1, const gsLargeInt_t* src2,
                       gsLargeInt_t* dest) {
  gsi_bool result = gsiLargeIntSub(src1->mData, src1->mLength, src2->mData,
                                   src2->mLength, dest->mData, &dest->mLength);
  if (gsi_is_true(result))
    gsiLargeIntStripLeadingZeroes(dest);
  return result;
}

gsi_bool gsiLargeIntSub(const l_word* src1, l_word length1, const l_word* src2,
                        l_word length2, l_word* dest, l_word* lenout) {
  ...
  while (i < shorterLen) {
    ...
    borrow =
        borrow >>
        63; // shift to last bit.  This will be 1 if negative, 0 if positive
    i++;
  }
  while (i < length2) {
...
gsi_bool gsLargeIntMult(const gsLargeInt_t* src1, const gsLargeInt_t* src2,
                        gsLargeInt_t* dest) {
  gsi_bool result =
      gsiLargeIntMult(src1->mData, src1->mLength, src2->mData, src2->mLength,
                      dest->mData, &dest->mLength, GS_LARGEINT_MAX_DIGITS);
  if (gsi_is_false(result))
    memset(dest, 0, sizeof(gsLargeInt_t)); // overflow
  return result;
...
gsi_bool gsLargeIntDiv(const gsLargeInt_t* src1, const gsLargeInt_t* divisor,
                       gsLargeInt_t* dest, gsLargeInt_t* remainder) {
...
static gsi_bool gsiLargeIntDiv(const l_word* src, l_word len,
                               const gsLargeInt_t* div, gsLargeInt_t* dest,
                               gsLargeInt_t* remainder) {
  gsi_i32 result = 0; // temp, to store compare result
  gsi_i32 divisorHighBit =
      GS_LARGEINT_DIGIT_SIZE_BITS - 1; // pre-calculate this
  ...
  l_word scopy[GS_LARGEINT_MAX_DIGITS * 2]; // we support double length source
                                            // for division, when dest is null
  l_word scopyLen = len;
  ...
  while ((divisorData[divisorLen - 1] & (1 << (gsi_u32)divisorHighBit)) == 0 &&
         divisorHighBit >= 0)
    divisorHighBit--;
  if (divisorHighBit == -1) {
    GSLINT_EXITTIMER(GSLintTimerDiv);
  ...
  do {
    result = gsiLargeIntCompare(&scopy[readIndex], (l_word)readLength,
                                divisorData, divisorLen);
    if (result == -1) {
      ...
      // subtract directly onto our temp copy, so we don't have to worry about
      // carry values
      l_word quotientTemp = 0;
      // if (readLength > 0xffff)
      //	_asm {int 3}
      if (gsi_is_false(gsiLargeIntSubDivide(
              &scopy[readIndex], (l_word)readLength, divisorData, divisorLen,
...
static gsi_bool gsiLargeIntSubDivide(l_word* src, l_word length,
                                     const l_word* divisor, l_word dlen,
                                     gsi_u32 highbit, l_word* quotient) {
  l_dword aboveBits = 0;
  gsLargeInt_t temp; // stores temporary product before subtraction
  gsLargeInt_t
      quotientCopy; // copy of quotient, length padded for multiplication
  ...
  if (length > dlen)
    aboveBits = (src[length - 2] & divisor[dlen - 1]) |
                ((l_dword)src[length - 1] << GS_LARGEINT_DIGIT_SIZE_BITS);
  else
  ...
  if (gsi_is_false(gsiLargeIntMult(divisor, dlen, quotientCopy.mData,
                                   quotientCopy.mLength, temp.mData,
...
gsi_bool gsLargeIntKMult(const gsLargeInt_t* src1, const gsLargeInt_t* src2,
                         gsLargeInt_t* dest) {
  l_word len = 0;
  ...
  if (src1->mLength != len || src2->mLength != len) {
    ...
    result = gsiLargeIntKMult(src1Copy.mData, src2Copy.mData, len, temp.mData,
                              &temp.mLength, GS_LARGEINT_MAX_DIGITS);
  } else {
    // size is correct, perform multiplication
    result = gsiLargeIntKMult(src1->mData, src2->mData, len, temp.mData,
                              &temp.mLength, GS_LARGEINT_MAX_DIGITS);
  }
  if (gsi_is_true(result)) {
...
static gsi_bool gsiLargeIntKMult(const l_word* data1, const l_word* data2,
  ...
  if (length <= GS_LARGEINT_KARATSUBA_CUTOFF)
    ...
    // Note that since (a*B^N + c) cannot overlap, we can immediately store both
    // in dest
    ...
    gsiLargeIntKMult(&data1[halfLen], &data2[halfLen], halfLen, &dest[length],
                     lenout, (l_word)(maxlen - length));
    ...
    gsiLargeIntAdd(&data1[halfLen], halfLen, data1, halfLen, temp1.mData,
                   &temp1.mLength, GS_LARGEINT_MAX_DIGITS);
    // printf("Calculated B1 (%d) = ", temp1.mLength);
    // gsiLargeIntPrint(temp1.mData, temp1.mLength);

    // Compute b2. (TH of data2 + BH of data2)
    gsiLargeIntAdd(&data2[halfLen], halfLen, data2, halfLen, temp2.mData,
                   &temp2.mLength, GS_LARGEINT_MAX_DIGITS);
    ...
    if (temp1.mLength > halfLen || temp2.mLength > halfLen)
      gsiLargeIntMult(temp1.mData, temp1.mLength, temp2.mData, temp2.mLength,
      ...
      gsiLargeIntKMult(temp1.mData, temp2.mData, *lenout, &temp3.mData[halfLen],
                       &temp3.mLength,
                       (l_word)(GS_LARGEINT_MAX_DIGITS - halfLen));
    }
    ...
    gsiLargeIntSub(&dest[length], length, &temp3.mData[halfLen],
                   (l_word)(temp3.mLength - halfLen), &temp3.mData[halfLen],
                   &temp3.mLength);
    temp3.mLength = (l_word)(temp3.mLength + halfLen);
    gsiLargeIntSub(dest, length, &temp3.mData[halfLen],
                   (l_word)(temp3.mLength - halfLen), &temp3.mData[halfLen],
                   &temp3.mLength);
    temp3.mLength = (l_word)(temp3.mLength + halfLen);
    ...
    gsiLargeIntAdd(dest, (l_word)(length * 2), temp3.mData, temp3.mLength, dest,
                   lenout, maxlen);
  }
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_bool gsLargeIntSquareMod(const gsLargeInt_t* lint, const gsLargeInt_t* mod,
                             gsLargeInt_t* dest) {
  int i = 0;
  int k = 0;
...
gsi_bool gsLargeIntPowerMod(const gsLargeInt_t* b, const gsLargeInt_t* p,
  ...
  if (gsiLargeIntCompare(base.mData, base.mLength, mod.mData, mod.mLength) !=
      -1) {
  ...
  expHighBit +=
      ((power.mLength - 1) *
       GS_LARGEINT_DIGIT_SIZE_BITS); // add in 32 bits for each extra byte
  ...
  while ((k > 1) && (gsi_u32)((k - 1) * (k << ((k - 1) << 1)) /
                              ((1 << k) - k - 1)) >= expHighBit - 1) {
    --k;
  }
  pow2k = 1 << k;
  kmask = pow2k - 1;
  ...
  */
  ...
  // if (k >= 3)
  {
    ...
    for (i = 2; i < valuesNeeded; i++) {
      if (gsi_is_false(gsiLargeIntMultM(&lut[1], &lut[i - 1], &mod, modPrime,
                                        &lut[i]))) {
        gsifree(lut);
        GSLINT_EXITTIMER(GSLintTimerPowerMod);
        return gsi_false;
  ...
  for (i = 0; i < kdigits; i++) {
    ...
    int l_index; // = ((bitReadIndex-1)/GS_LARGEINT_DIGIT_SIZE_BITS); // -1 to
    ...
    l_index = ((bitReadIndex - 1) /
               GS_LARGEINT_DIGIT_SIZE_BITS); // -1 to use zero based indexes

    // for first digit, use leading zeroes when necessary
    if ((bitReadIndex % k) != 0)
      bitReadIndex += k - (bitReadIndex % k); // round up to next k
    if (i != 0) {
      if (bitReadIndex - (l_index * GS_LARGEINT_DIGIT_SIZE_BITS) >
    ...
    if (i == 0) {
      // first digit
      l_firstbit =
          l_index * GS_LARGEINT_DIGIT_SIZE_BITS; // first bit of this digit
      twodigits = p->mData[l_index];
    } else if (l_index > 0) {
      // middle digits
      l_firstbit = (l_index - 1) *
                   GS_LARGEINT_DIGIT_SIZE_BITS; // first bit of this digit
      twodigits =
      ...
      l_firstbit = 0;
      twodigits =
          (l_dword)(p->mData[l_index + 1] << GS_LARGEINT_DIGIT_SIZE_BITS) |
          p->mData[l_index];
    } else {
    ...
    // use digitval to determine how many squaring and multiplication operations
    // we need to perform
    {
      static int twotab[] = {
      ...
      static USHORT oddtab[] = {
          0,   1,   1,   3,   1,   5,   3,   7,   1,   9,   5,   11,  3,   13,
          7,   15,  1,   17,  9,   19,  5,   21,  11,  23,  3,   25,  13,  27,
          7,   29,  15,  31,  1,   33,  17,  35,  9,   37,  19,  39,  5,   41,
          21,  43,  11,  45,  23,  47,  3,   49,  25,  51,  13,  53,  27,  55,
          7,   57,  29,  59,  15,  61,  31,  63,  1,   65,  33,  67,  17,  69,
          35,  71,  9,   73,  37,  75,  19,  77,  39,  79,  5,   81,  41,  83,
          21,  85,  43,  87,  11,  89,  45,  91,  23,  93,  47,  95,  3,   97,
          49,  99,  25,  101, 51,  103, 13,  105, 53,  107, 27,  109, 55,  111,
          7,   113, 57,  115, 29,  117, 59,  119, 15,  121, 61,  123, 31,  125,
          63,  127, 1,   129, 65,  131, 33,  133, 67,  135, 17,  137, 69,  139,
          35,  141, 71,  143, 9,   145, 73,  147, 37,  149, 75,  151, 19,  153,
          77,  155, 39,  157, 79,  159, 5,   161, 81,  163, 41,  165, 83,  167,
          21,  169, 85,  171, 43,  173, 87,  175, 11,  177, 89,  179, 45,  181,
          91,  183, 23,  185, 93,  187, 47,  189, 95,  191, 3,   193, 97,  195,
          49,  197, 99,  199, 25,  201, 101, 203, 51,  205, 103, 207, 13,  209,
          105, 211, 53,  213, 107, 215, 27,  217, 109, 219, 55,  221, 111, 223,
          7,   225, 113, 227, 57,  229, 115, 231, 29,  233, 117, 235, 59,  237,
          119, 239, 15,  241, 121, 243, 61,  245, 123, 247, 31,  249, 125, 251,
          63,  253, 127, 255};
      ...
      if (i == 0) {
        ...
        // int lutindex = (oddtab[digitval]+1)/2; // we only precalculate the
        // odd powers
        ...
        if (gsi_is_false(
                gsiLargeIntMultM(dest, &lut[lutindex], &mod, modPrime, dest))) {
          gsifree(lut);
          return gsi_false;
        }
        // printf("[gsint]    Mult by [%d](%d) to %d\r\n", lutindex,
        // lut[lutindex].mData[0], dest->mData[0]);
        for (counter = twotab[digitval]; counter > 0; counter--) {
          if (gsi_is_false(
                  gsiLargeIntMultM(dest, dest, &mod, modPrime, dest))) {
            gsifree(lut);
...
gsi_bool gsLargeIntPowerMod(const gsLargeInt_t* b, const gsLargeInt_t* p,
  ...
  //       ex. (1383 mod 16) = binary(0000010101100111 mod 00010000) = 00000111
  //       = dec 7
  ...
  //    TODO:  THIS DOESN'T WORK IF THE HIGHBIT IS EVER ABOVE
  //    GS_LARGEINT_DIGIT_SIZE_BITS
...
gsi_bool gsiLargeIntMultM(gsLargeInt_t* x, gsLargeInt_t* y,
                          const gsLargeInt_t* m, gsi_u32 modPrime,
                          gsLargeInt_t* dest) {
  l_word tempLen = 0;
  ...
  if (gsi_is_false(gsiLargeIntMult(x->mData, x->mLength, y->mData, y->mLength,
                                   temp, &tempLen, GS_LARGEINT_MAX_DIGITS * 2)))
    return gsi_false;
  ...
  if (tempLen < m->mLength * 2) {
    memset(&temp[tempLen], 0,
           (m->mLength * 2 - tempLen) * GS_LARGEINT_DIGIT_SIZE_BYTES);
    // memset(&temp[tempLen], 0, sizeof(temp) - tempLen *
    // GS_LARGEINT_DIGIT_SIZE_BYTES); // safer to clear out the whole thing?
  ...
  // These bytes should be cleared to prevent bugs where that data may be
  // exposed.  (buffer overrun?)
  if (gsiLargeIntCompare(&temp[logB_r], tempLen - logB_r, m->mData,
                         m->mLength) != -1) {
    if (gsi_is_false(gsiLargeIntSub(m->mData, m->mLength, &temp[logB_r],
                                    tempLen - logB_r, dest->mData,
                                    &dest->mLength))) {
      memset(temp, 0, sizeof(temp));
    ...
    memcpy(dest->mData, &temp[logB_r],
           (tempLen - logB_r) * GS_LARGEINT_DIGIT_SIZE_BYTES);
    memset(temp, 0, sizeof(temp));
  }
...
//    Computes (src1*src2*r^-1)%mod
// Note:
//    This implementation is based on HAC14.36 which has a lot of room for
//    improvement FLINT algorithm runs approx 30 times faster.
gsi_bool gsiLargeIntMultM(gsLargeInt_t* x, gsLargeInt_t* y,
                          const gsLargeInt_t* m, gsi_u32 modPrime,
                          gsLargeInt_t* dest) {
  int i = 0;
  ...
  // We pad with zeroes so that we don't have to check for overruns in the loop
  // below
  ...
  for (i = 0; (gsi_u32)i < m->mLength; i++) {
    ...
    u = (l_word)((xiy0 + A.mData[0]) *
                 modPrime); // strip bits over the first digit

    // A = (A+x[i]*y + u[i]*m)/b
    //    compute x[i]*y
    memset(temp.mData, 0,
           y->mLength * sizeof(l_word)); // clear out a portion of temp
    temp.mData[0] = x->mData[i];
    temp.mLength = y->mLength; // xi padded with zeroes
    if (gsi_is_false(gsiLargeIntMult(temp.mData, temp.mLength, y->mData,
                                     y->mLength, xiy.mData, &xiy.mLength,
    ...
    //    compute u[i]*m
    memset(temp.mData, 0,
           m->mLength * sizeof(l_word)); // clear out a portion of temp
    temp.mData[0] = u;
    temp.mLength = m->mLength;
    // if (gsi_is_false(gsiLargeIntMult(temp.mData, temp.mLength, m->mData,
    // m->mLength, temp.mData, &temp.mLength)))
    ...
    //    Add both to A
    if (gsi_is_false(gsiLargeIntAdd(xiy.mData, xiy.mLength, A.mData, A.mLength,
                                    A.mData, &A.mLength,
                                    GS_LARGEINT_MAX_DIGITS))) {
      // overflow
      dest->mLength = 0;
    ...
    if (gsi_is_false(gsiLargeIntAdd(temp.mData, temp.mLength, A.mData,
                                    A.mLength, A.mData, &A.mLength,
                                    GS_LARGEINT_MAX_DIGITS))) {
      // overflow
      dest->mLength = 0;
      dest->mData[0] = 0;
  ...
  if (gsiLargeIntCompare(A.mData, A.mLength, m->mData, m->mLength) != -1)
    gsiLargeIntSub(m->mData, m->mLength, A.mData, A.mLength, dest->mData,
                   &dest->mLength);
  else
...
gsi_bool gsLargeIntSetFromHexString(gsLargeInt_t* lint, const char* hexstream) {
  ...
  lint->mLength = (l_word)((len + (2 * GS_LARGEINT_DIGIT_SIZE_BYTES - 1)) /
                           (2 * GS_LARGEINT_DIGIT_SIZE_BYTES));
  lint->mData[lint->mLength - 1] =
      0; // set last byte to zero for left over characters

  while (len > 0) {
    if (len >= 2)
      sscanf((char*)(hexstream + len - 2), "%02x",
             &temp); // sscanf requires a 4 byte dest
    else
      sscanf((char*)(hexstream + len - 1), "%01x",
             &temp); // sscanf requires a 4 byte dest
    if (byteIndex == 0)
      *writePos = 0;
...
void gsLargeIntAddToMD5(const gsLargeInt_t* _lint, MD5_CTX* md5) {
  ...
  if ((byteLength % GS_LARGEINT_DIGIT_SIZE_BYTES) != 0)
    dataStart += GS_LARGEINT_DIGIT_SIZE_BYTES -
                 (byteLength % GS_LARGEINT_DIGIT_SIZE_BYTES);
