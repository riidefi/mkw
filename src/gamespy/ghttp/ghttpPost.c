...
#define GHI_LEGAL_URLENCODED_CHARS                                             \
...
GHTTPBool ghiPostAddString(GHTTPPost _post, const char* name,
                           const char* string) {
  GHIPost* post = (GHIPost*)_post;
  GHIPostData data;
  int len;
...
GHTTPBool ghiPostAddFileFromDisk(GHTTPPost _post, const char* name,
                                 const char* filename,
...
GHTTPBool ghiPostAddFileFromMemory(GHTTPPost _post, const char* name,
...
void ghiPostSetCallback(GHTTPPost _post, ghttpPostCallback callback,
                        void* param) {
  GHIPost* post = (GHIPost*)_post;
...
static int ghiPostGetNoFilesContentLength(struct GHIConnection* connection) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (data->type == GHIString) {
      ...
      GS_ASSERT(foundSoapAlready ==
                0); // only support one soap object per request
      foundSoapAlready = 1;
...
static int ghiPostGetHasFilesContentLength(struct GHIConnection* connection) {
  ...
  if (!boundaryLen) {
    if (post->useDIME) {
      ...
      fileBaseLen =
          (boundaryLen + 76); // + name + filename + content-type + file
...
static GHTTPBool ghiPostStateInit(GHIPostState* state) {
  ...
  if (type == GHIString) {
    ...
    /////////////////
#ifndef NOFILE
    state->state.fileDisk.file =
        fopen(state->data->data.fileDisk.filename, "rb");
#endif
...
GHTTPBool ghiPostInitState(struct GHIConnection* connection) {
  ...
  // If this asserts, there aren't the same number of state objects as data
  // objects. There should be a 1-to-1 mapping between data and states.
  //////////////////////////////////////////////////////////////////////////////////
  // assert(ArrayLength(connection->post->data) ==
...
static GHIPostingResult ghiPostStringStateDoPosting(GHIPostState* state,
  ...
  if (!connection->post->hasFiles && !connection->post->hasSoap &&
      state->data->data.string.invalidChars) {
    int i;
    int c;
    ...
    GHITrySendResult result =
        ghiTrySendThenBuffer(connection, state->data->data.string.string,
                             state->data->data.string.len);
...
static GHIPostingResult ghiPostXmlStateDoPosting(GHIPostState* state,
                                                 GHIConnection* connection) {
  ...
  if (connection->encryptor.mEngine != GHTTPEncryptionEngine_None &&
    ...
    if (!ghiAppendDataToBuffer(&connection->encodeBuffer,
                               gsXmlWriterGetData(xml),
                               gsXmlWriterGetDataLength(xml)) ||
        !ghiAppendDataToBuffer(&connection->encodeBuffer, pad, padlen) ||
        !ghiEncryptDataToBuffer(&connection->sendBuffer,
                                connection->encodeBuffer.data,
...
ghiPostFileDiskStateDoPosting(GHIPostState* state, GHIConnection* connection) {
  char buffer[4096];
  int len;
  GHITrySendResult result;
...
static GHIPostingResult
ghiPostFileMemoryStateDoPosting(GHIPostState* state,
                                GHIConnection* connection) {
  int rcode;
  int len;
  ...
  ////////////////////
  if (connection->encryptor.mEngine == GHTTPEncryptionEngine_None ||
      connection->encryptor.mEncryptOnBuffer == NULL) {
    ...
    do {
      ...
      if (state->data->data.fileMemory.len == state->pos) {
        ...
        if (connection->post->useDIME) {
          ...
          if (padlen != 4 && padlen > 0) {
            if (GHITrySendError ==
                ghiTrySendThenBuffer(connection, pad, padlen))
              return GHIPostingError;
          }
        }
        return GHIPostingDone;
    ...
    do {
      ...
      result = ghiTrySendThenBuffer(
          connection, state->data->data.fileMemory.buffer + state->pos, len);
      if (result == GHITrySendError)
        return GHIPostingError;
...
static GHIPostingResult ghiPostStateDoPosting(GHIPostState* state,
                                              GHIConnection* connection,
                                              GHTTPBool first, GHTTPBool last) {
  int len = 0;
  GHITrySendResult result;
    ...
    if (!connection->post->hasFiles && !connection->post->hasSoap) {
      ...
      if (state->data->type == GHIString) {
        sprintf(buffer,
                ...
                "name=\"%s\"" CRLF CRLF,
                first ? GHI_MULTIPART_BOUNDARY_FIRST
                      : GHI_MULTIPART_BOUNDARY_NORMAL,
                state->data->name);
        ...
        if (connection->post->useDIME) {
          ...
          header.mDataLength =
              htonl(gsXmlWriterGetDataLength(state->data->data.xml.xml));
          ...
          buffer[0] = '\0';
      } else if ((state->data->type == GHIFileDisk) ||
                 (state->data->type == GHIFileMemory)) {
        const char* filename;
        const char* contentType;
        int filelen;
        ...
        if (connection->post->useDIME) {
          ...
          sprintf(buffer,
                  ...
                  first ? GHI_MULTIPART_BOUNDARY_FIRST
                        : GHI_MULTIPART_BOUNDARY_NORMAL,
                  state->data->name, filename, contentType);
        }
      } else {
        // assert(0);
      }
...
GHIPostingResult ghiPostDoPosting(struct GHIConnection* connection) {
  ...
  // assert(connection->postingState.index <=
  //        ArrayLength(connection->postingState.states));
  ...
  while (postingState->index < len) {
    ...
    //////////////////////////////
    postState =
        (GHIPostState*)ArrayNth(postingState->states, postingState->index);
    // assert(postState);

    // Upload the current data.
    ///////////////////////////
    postingResult = ghiPostStateDoPosting(
        postState, connection,
        (postingState->index == 0) ? GHTTPTrue : GHTTPFalse,
        (postingState->index == (ArrayLength(postingState->states) - 1))
            ? GHTTPTrue
            : GHTTPFalse);
  ...
  //   -- for example, when posting string data we don't encrypt until we have
  //   the entire string (for efficiency only)
  if (connection->encryptor.mEngine != GHTTPEncryptionEngine_None) {
    if (connection->encodeBuffer.len > 0) {
      GS_ASSERT(connection->encodeBuffer.pos ==
                0); // if you hit this, it means you forgot the clear the buffer
  ...
  if (connection->post->hasFiles && !connection->post->useDIME) {
    ...
    trySendResult =
        ghiTrySendThenBuffer(connection, GHI_MULTIPART_BOUNDARY_END,
                             (int)strlen(GHI_MULTIPART_BOUNDARY_END));
    if (trySendResult == GHITrySendError)
      return GHIPostingError;
  }
