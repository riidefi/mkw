...
static int GS_STATIC_CALLBACK gti2ConnectionCompare(const void* elem1,
                                                    const void* elem2) {
...
GT2Connection gti2SocketFindConnection(GT2Socket socket, unsigned int ip,
                                       unsigned short port) {
  ...
  connectionPtrPtr =
      (GT2Connection*)TableLookup(socket->connections, &connectionPtr);
  if (connectionPtrPtr)
...
GT2Result gti2CreateSocket(GT2Socket* sock, const char* localAddress,
                           int outgoingBufferSize, int incomingBufferSize,
  ...
  socketTemp->connections =
      TableNew2(sizeof(GT2Connection), 32, 2, gti2ConnectionHash,
                gti2ConnectionCompare, NULL);
  ...
  socketTemp->closedConnections =
      ArrayNew(sizeof(GT2Connection), 4, gti2ConnectionFree);
...
static GT2Connection gti2CreateConnectionObject(void) {
  // mj todo: give options of allocating this from a static pool for games with
  // known number of connections.
  return (GT2Connection)gsimalloc(sizeof(GTI2Connection));
}

GT2Result gti2NewSocketConnection(GT2Socket socket, GT2Connection* connection,
  ...
  if (!gti2AllocateBuffer(&connectionPtr->incomingBuffer,
                          socket->incomingBufferSize))
    goto out_of_memory;
  if (!gti2AllocateBuffer(&connectionPtr->outgoingBuffer,
                          socket->outgoingBufferSize))
    goto out_of_memory;

  // allocate the message arrays
  connectionPtr->incomingBufferMessages =
      ArrayNew(sizeof(GTI2IncomingBufferMessage), 64, NULL);
  if (!connectionPtr->incomingBufferMessages)
    goto out_of_memory;
  connectionPtr->outgoingBufferMessages =
      ArrayNew(sizeof(GTI2OutgoingBufferMessage), 64, NULL);
  ...
  if (!connectionPtr->sendFilters)
    goto out_of_memory;
  connectionPtr->receiveFilters =
      ArrayNew(sizeof(gt2ReceiveFilterCallback), 2, NULL);
...
void gti2FreeSocketConnection(GT2Connection connection) {
  ...
  if (connection->state == GTI2Closed) {
    ...
    for (i = 0; i < len; i++) {
      if (connection ==
          *(GT2Connection*)ArrayNth(connection->socket->closedConnections, i)) {
...
GT2Bool gti2SocketSend(GT2Socket socket, unsigned int ip, unsigned short port,
  ...
  address.sin_addr.s_addr = ip;
  address.sin_port = htons(port);
  rcode = sendto(socket->socket, (const char*)message, len, 0,
...
static int gti2SocketConnectionsThinkMap(void* elem, void* clientData) {
  ...
  if ((connection->state == GTI2Closed) && !connection->freeAtAcceptReject &&
...
void gti2FreeClosedConnections(GT2Socket socket) {
  ...
  for (i = (len - 1); i >= 0; i--)
    gti2FreeSocketConnection(
        *(GT2Connection*)ArrayNth(socket->closedConnections, i));
}
