...
static GPResult gpiFinishTransferMessage(GPConnection* connection,
  ...
  GPResult result =
      gpiPeerFinishTransferMessage(connection, transfer->peer, message, len);
...
static GPResult gpiNewTransfer(GPConnection* connection, GPITransfer** transfer,
                               GPProfile profile, GPIBool sender) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  ...
  // Get it.
  //////////
  *transfer = (GPITransfer*)ArrayNth(iconnection->transfers,
                                     ArrayLength(iconnection->transfers) - 1);

  return GP_NO_ERROR;
}

GPResult gpiNewSenderTransfer(GPConnection* connection, GPITransfer** transfer,
                              GPProfile profile) {
...
static GPResult gpiNewReceiverTransfer(GPConnection* connection,
                                       GPITransfer** transfer,
                                       GPProfile profile,
...
void gpiCancelTransfer(GPConnection* connection, GPITransfer* transfer) {
  ...
  if (transfer->peer) {
    ...
    if (gpiPeerStartTransferMessage(
            connection, transfer->peer, GPI_BM_FILE_TRANSFER_CANCEL,
...
GPIFile* gpiAddFileToTransfer(GPITransfer* transfer, const char* path,
                              const char* name) {
  GPIFile file;
  char* str;
...
static GPResult gpiSendTransferRequest(GPConnection* connection,
                                       GPITransfer* transfer) {
  char buffer[32];
  GPIFile* file;
  int i;
  ...
  CHECK_RESULT(gpiPeerStartTransferMessage(
      connection, transfer->peer, GPI_BM_FILE_SEND_REQUEST,
      (GPITransferID_st)&transfer->transferID));
  ...
  for (i = 0; i < num; i++) {
    ...
    CHECK_RESULT(gpiSendOrBufferString(connection, transfer->peer, buffer));
    CHECK_RESULT(
        gpiSendOrBufferUInt(connection, transfer->peer, file->modTime));
  }

  // Finish the message.
  //////////////////////
  CHECK_RESULT(
      gpiFinishTransferMessage(connection, transfer, transfer->message, -1));

  return GP_NO_ERROR;
}

static GPITransfer* gpiFindTransferByTransferID(GPConnection* connection,
                                                GPITransferID* transferID) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  int i;
  int num;
...
void gpiSkipFile(GPConnection* connection, GPITransfer* transfer, int file,
                 int reason) {
  char buffer[32];

  if (gpiPeerStartTransferMessage(connection, transfer->peer, GP_FILE_SKIP,
                                  (GPITransferID_st)&transfer->transferID) !=
      GP_NO_ERROR)
    return;
...
void gpiSkipCurrentFile(GPConnection* connection, GPITransfer* transfer,
                        int reason) {
...
static GPIBool gpiHandleSendRequest(GPConnection* connection, GPIPeer* peer,
                                    GPITransferID* transferID,
  ...
  //////////////////////////////
  if (gpiNewReceiverTransfer(connection, &transfer, peer->profile,
                             transferID) != GP_NO_ERROR)
    return GPIFalse;
...
static GPIBool gpiHandleSendReply(GPConnection* connection,
                                  GPITransfer* transfer, const char* headers,
...
static GPIBool gpiHandleBegin(GPConnection* connection, GPITransfer* transfer,
  ...
  count = 0;
  do {
    sprintf(buffer, "%sgpt_%d_%d_%d.gpt", transfer->baseDirectory,
            transfer->localID, fileIndex, rand());
...
static GPIBool gpiHandleEnd(GPConnection* connection, GPITransfer* transfer,
                            const char* headers) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  GPTransferCallbackArg* arg;
  GPIFile* file;
  ...
  if (file->flags & GPI_FILE_DIRECTORY) {
    ...
    // Send a confirmation.
    ///////////////////////
    if (gpiPeerStartTransferMessage(connection, transfer->peer, GPI_BM_FILE_END,
                                    (GPITransferID_st)&transfer->transferID) !=
        GP_NO_ERROR)
      return GPIFalse;
    gpiFinishTransferMessage(connection, transfer, NULL, 0);
...
static GPIBool gpiHandleData(GPConnection* connection, GPITransfer* transfer,
                             const char* headers, const char* buffer,
                             int bufferLen) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  GPTransferCallbackArg* arg;
  ...
  //////////////////
  writeFailed =
      (GPIBool)(fwrite(buffer, 1, bufferLen, file->file) != (size_t)bufferLen);
  if (writeFailed) {
    // Flag the errors.
    ///////////////////
    file->flags |= GPI_FILE_FAILED;
    ...
    //////////////////////////
    if (gpiPeerStartTransferMessage(
            connection, transfer->peer, GPI_BM_FILE_DATA,
            (GPITransferID_st)&transfer->transferID) != GP_NO_ERROR)
      return GPIFalse;
...
static GPIBool gpiHandleSkip(GPConnection* connection, GPITransfer* transfer,
                             const char* headers) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  GPTransferCallbackArg* arg;
  GPIFile* file;
...
static GPIBool gpiHandleTransferThrottle(GPConnection* connection,
                                         GPITransfer* transfer,
                                         const char* headers) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  int throttle;
  ...
  if (transfer->sender) {
    if (gpiPeerStartTransferMessage(
            connection, transfer->peer, GPI_BM_FILE_TRANSFER_THROTTLE,
            (GPITransferID_st)&transfer->transferID) != GP_NO_ERROR)
      return GPIFalse;
...
static GPIBool gpiHandleTransferCancel(GPConnection* connection,
                                       GPITransfer* transfer) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  GPTransferCallbackArg* arg;
...
static GPResult gpiSendFileEnd(GPConnection* connection, GPITransfer* transfer,
                               GPIFile* file) {
  CHECK_RESULT(
      gpiPeerStartTransferMessage(connection, transfer->peer, GPI_BM_FILE_END,
                                  (GPITransferID_st)&transfer->transferID));
...
static GPResult gpiSendFileBegin(GPConnection* connection,
                                 GPITransfer* transfer, GPIFile* file) {
  char buffer[64];
  ...
  CHECK_RESULT(
      gpiPeerStartTransferMessage(connection, transfer->peer, GPI_BM_FILE_BEGIN,
                                  (GPITransferID_st)&transfer->transferID));
  sprintf(buffer, "\\file\\%d\\size\\%d\\mtime\\%u", transfer->currentFile,
          file->size, (unsigned int)file->modTime);
  gpiSendOrBufferString(connection, transfer->peer, buffer);
  gpiFinishTransferMessage(connection, transfer, NULL, 0);
...
static GPResult gpiSendFileData(GPConnection* connection, GPITransfer* transfer,
                                unsigned char* data, size_t len) {
  CHECK_RESULT(
      gpiPeerStartTransferMessage(connection, transfer->peer, GPI_BM_FILE_DATA,
                                  (GPITransferID_st)&transfer->transferID));
...
GPResult gpiProcessCurrentFile(GPConnection* connection,
                               GPITransfer* transfer) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  GPIFile* file;
  GPTransferCallbackArg* arg;
    ...
    if (file->flags & GPI_FILE_DIRECTORY) {
      ...
      if (arg) {
        ...
        gpiAddCallback(connection,
                       iconnection->callbacks[GPI_TRANSFER_CALLBACK], arg, NULL,
                       GPI_ADD_TRANSFER_CALLBACK);
      }
      ...
      if (!file->file) {
        ...
        if (file->file) {
          ...
          /////////////////////
          arg =
              (GPTransferCallbackArg*)gsimalloc(sizeof(GPTransferCallbackArg));
          if (arg) {
            memset(arg, 0, sizeof(GPTransferCallbackArg));
            arg->transfer = transfer->localID;
            arg->index = transfer->currentFile;
            arg->type = GP_FILE_BEGIN;
            gpiAddCallback(connection,
                           iconnection->callbacks[GPI_TRANSFER_CALLBACK], arg,
                           NULL, GPI_ADD_TRANSFER_CALLBACK);
          }
        } else {
          // Call the callback.
          /////////////////////
      ...
      ///////////////////////////////////////////////////////////////
      total = 0;
      for (i = 0; (file->progress < file->size) &&
                  !transfer->peer->outputBuffer.len /*&& (i < 20)*/;
           i++) {
#ifdef GPI_ACKNOWLEDGED_WINDOW
        ...
        if (num) {
          ...
          // Send the data.
          /////////////////
          CHECK_RESULT(gpiSendFileData(connection, transfer,
...
GPResult gpiProcessTransfer(GPConnection* connection, GPITransfer* transfer) {
  ...
  if (!transfer->peer) {
    ...
    if ((now - transfer->lastSend) > GPI_KEEPALIVE_TIME) {
      ...
      ////////////////////
      CHECK_RESULT(gpiPeerStartTransferMessage(
          connection, transfer->peer, GPI_BM_FILE_TRANSFER_KEEPALIVE,
          (GPITransferID_st)&transfer->transferID));
      gpiFinishTransferMessage(connection, transfer, NULL, 0);
    }
  }
...
void gpiTransfersHandlePong(GPConnection* connection, GPProfile profile,
                            GPIPeer* peer) {
  GPIConnection* iconnection = (GPIConnection*)*connection;
  GPITransfer* transfer;
  int i;
  ...
  for (i = 0; i < len; i++) {
    ...
    /////////////////////////////////////////////
    if ((transfer->state == GPITransferPinging) &&
...
GPResult gpiSendTransferReply(GPConnection* connection,
                              const GPITransferID* transferID, GPIPeer* peer,
  ...
  // Start the message.
  /////////////////////
  CHECK_RESULT(gpiPeerStartTransferMessage(connection, peer,
                                           GPI_BM_FILE_SEND_REPLY, transferID));
...
void gpiHandleTransferMessage(GPConnection* connection, GPIPeer* peer, int type,
  ...
  if (!gpiValueForKey(headers, "\\xfer\\", value, sizeof(value)))
    return;
  if (sscanf(value, "%d %u %u", &transferID.profileid, &transferID.count,
             &transferID.time) != 3)
    return;
  ...
  if (type == GPI_BM_FILE_SEND_REQUEST) {
    ...
    ///////////////////////////////////////
    if (!gpiHandleSendRequest(connection, peer, &transferID, headers, buffer,
                              len))
      gpiSendTransferReply(connection, &transferID, peer, GPI_NOT_ACCEPTING,
                           NULL);
