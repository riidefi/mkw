...
///////////
static GPResult
gpiSendRegisterUniqueNick(GPConnection* connection,
                          const char uniquenick[GP_UNIQUENICK_LEN],
                          const char cdkey[GP_CDKEY_LEN], int operationid) {
  GPIConnection* iconnection = (GPIConnection*)*connection;

  gpiAppendStringToBuffer(connection, &iconnection->outputBuffer,
                          "\\registernick\\\\sesskey\\");
...
GPResult gpiRegisterUniqueNick(GPConnection* connection,
  ...
  // Add the operation.
  /////////////////////
  CHECK_RESULT(gpiAddOperation(connection, GPI_REGISTER_UNIQUENICK, NULL,
                               &operation, blocking, callback, param));

  // Send a request for info.
  ///////////////////////////
  result =
      gpiSendRegisterUniqueNick(connection, uniquenick, cdkey, operation->id);
  CHECK_RESULT(result);
...
GPResult gpiProcessRegisterUniqueNick(GPConnection* connection,
  ...
  if (callback.callback != NULL) {
    ...
    arg = (GPRegisterUniqueNickResponseArg*)gsimalloc(
        sizeof(GPRegisterUniqueNickResponseArg));
    if (arg == NULL)
...
static GPResult gpiSendRegisterCdKey(GPConnection* connection,
  ...
  //	gpiAppendStringToBuffer(connection, &iconnection->outputBuffer,
  //"\\partnerid\\"); 	gpiAppendIntToBuffer(connection,
  //&iconnection->outputBuffer, iconnection->partnerID);
  gpiAppendStringToBuffer(connection, &iconnection->outputBuffer, "\\id\\");
  gpiAppendIntToBuffer(connection, &iconnection->outputBuffer, operationid);
  gpiAppendStringToBuffer(connection, &iconnection->outputBuffer, "\\final\\");
...
GPResult gpiRegisterCdKey(GPConnection* connection,
  ...
  // Add the operation.
  /////////////////////
  CHECK_RESULT(gpiAddOperation(connection, GPI_REGISTER_CDKEY, NULL, &operation,
...
GPResult gpiProcessRegisterCdKey(GPConnection* connection,
  ...
  if (callback.callback != NULL) {
    ...
    arg = (GPRegisterCdKeyResponseArg*)gsimalloc(
        sizeof(GPRegisterCdKeyResponseArg));
    if (arg == NULL)
      Error(connection, GP_MEMORY_ERROR, "Out of memory.");
