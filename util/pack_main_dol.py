...
def elf_to_dol(elf_file_path, dol_file_path):
    ...
    with open(elf_file_path, "rb") as elf_raw_file, open(dol_file_path, "wb") as dol_file:
...
with open(args.input, "r+b") as elff:
    elff.seek(0x18)
    elff.write(bytes([0x80, 0x00, 0x60, 0xA4]))
elf_to_dol(args.input, args.out)
