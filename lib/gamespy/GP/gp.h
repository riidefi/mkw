...
typedef struct {
  ...
  float
      latitude; // negative is south, positive is north.  (0, 0) means unknown.
  gsi_char place[GP_PLACE_LEN]; // e.g., "USA|California|Irvine", "South
                                // Korea|Seoul", "Turkey"
  int birthday;
  int birthmonth;
  int birthyear;
...
GPResult gpInitialize(
    ...
    int namespaceID, // The namespaceID identified which namespace to login
                     // under. A namespaceID of 0 indicates that no namespace
                     // should be used. A namespaceID of 1 represents the
                     // default GameSpy namespace
...
// gpConnect
////////////
GPResult gpConnect(GPConnection* connection, const gsi_char nick[GP_NICK_LEN],
                   const gsi_char email[GP_EMAIL_LEN],
                   const gsi_char password[GP_PASSWORD_LEN], GPEnum firewall,
...
GPResult gpConnectNewUser(GPConnection* connection,
                          ...
                          const gsi_char cdkey[GP_CDKEY_LEN], GPEnum firewall,
                          GPEnum blocking, GPCallback callback, void* param);
...
GPResult gpConnectPreAuthenticated(
    GPConnection* connection, const gsi_char authtoken[GP_AUTHTOKEN_LEN],
    const gsi_char partnerchallenge[GP_PARTNERCHALLENGE_LEN], GPEnum firewall,
    GPEnum blocking, GPCallback callback, void* param);
...
// gpCheckUser
//////////////
GPResult gpCheckUser(GPConnection* connection, const gsi_char nick[GP_NICK_LEN],
                     const gsi_char email[GP_EMAIL_LEN],
                     const gsi_char password[GP_PASSWORD_LEN], GPEnum blocking,
                     GPCallback callback, void* param);
...
GPResult gpNewUser(GPConnection* connection, const gsi_char nick[GP_NICK_LEN],
                   ...
                   const gsi_char email[GP_EMAIL_LEN],
                   const gsi_char password[GP_PASSWORD_LEN],
                   const gsi_char cdkey[GP_CDKEY_LEN], GPEnum blocking,
                   GPCallback callback, void* param);
...
// gpGetErrorString
///////////////////
GPResult gpGetErrorString(GPConnection* connection,
                          gsi_char errorString[GP_ERROR_STRING_LEN]);

// gpNewProfile
///////////////
GPResult gpNewProfile(GPConnection* connection,
                      const gsi_char nick[GP_NICK_LEN], GPEnum replace,
                      GPEnum blocking, GPCallback callback, void* param);
...
GPResult gpProfileSearch(GPConnection* connection,
                         ...
                         const gsi_char lastname[GP_LASTNAME_LEN], int icquin,
                         GPEnum blocking, GPCallback callback, void* param);
...
// gpSetInfos
/////////////
GPResult gpSetInfos(GPConnection* connection, GPEnum info,
...
// gpSendBuddyRequest
/////////////////////
GPResult gpSendBuddyRequest(GPConnection* connection, GPProfile profile,
...
#ifdef GP_NEW_STATUS_INFO
GPResult gpSetStatusInfo(GPConnection* connection, GPEnum statusState,
                         unsigned int hostIp, unsigned int hostPrivateIp,
                         unsigned short queryPort, unsigned short hostPort,
                         unsigned int sessionFlags, const gsi_char* richStatus,
                         int richStatusLen, const gsi_char* gameType,
                         int gameTypeLen, const gsi_char* gameVariant,
                         int gameVariantLen, const gsi_char* gameMapName,
                         int gameMapNameLen);

GPResult gpAddStatusInfoKey(GPConnection* connection, const gsi_char* keyName,
                            const gsi_char* keyValue);
GPResult gpSetStatusInfoKey(GPConnection* connection, const gsi_char* keyName,
                            const gsi_char* keyValue);
GPResult gpGetStatusInfoKeyVal(GPConnection* connection,
...
// gpSendBuddyMessage
/////////////////////
GPResult gpSendBuddyMessage(GPConnection* connection, GPProfile profile,
                            const gsi_char* message);

// gpSendBuddyUTM
/////////////////////
GPResult gpSendBuddyUTM(GPConnection* connection, GPProfile profile,
                        const gsi_char* message,
...
GPResult gpIsValidEmail(GPConnection* connection,
                        const gsi_char email[GP_EMAIL_LEN], GPEnum blocking,
                        GPCallback callback, void* param);
...
// gpInvitePlayer
/////////////////
GPResult gpInvitePlayer(GPConnection* connection, GPProfile profile,
                        int productID,
...
// gpSetCdKey
GPResult gpSetCdKey(GPConnection* connection, const gsi_char cdkeyhash,
                    GPCallback callback);
...
// FILE TRANSFER //
///////////////////
typedef void (*gpSendFilesCallback)(GPConnection* connection, int index,
                                    const gsi_char** path,
                                    const gsi_char** name, void* param);

GPResult gpSendFiles(GPConnection* connection, GPTransfer* transfer,
                     GPProfile profile, const gsi_char* message,
                     gpSendFilesCallback callback, void* param);

GPResult gpAcceptTransfer(GPConnection* connection, GPTransfer transfer,
                          const gsi_char* message);

GPResult gpRejectTransfer(GPConnection* connection, GPTransfer transfer,
                          const gsi_char* message);
...
GPResult gpSetTransferDirectory(GPConnection* connection, GPTransfer transfer,
