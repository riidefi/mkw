...
GT2Result gti2NewOutgoingConnection(GT2Socket socket, GT2Connection* connection,
...
GT2Result gti2NewIncomingConnection(GT2Socket socket, GT2Connection* connection,
                                    unsigned int ip, unsigned short port) {
  GT2Result result;
...
GT2Bool gti2AcceptConnection(GT2Connection connection,
                             GT2ConnectionCallbacks* callbacks) {
...
void gti2RejectConnection(GT2Connection connection, const GT2Byte* message,
                          int len) {
  // make sure this flag gets cleared
  connection->freeAtAcceptReject = GT2False;
...
GT2Bool gti2ConnectionSendData(GT2Connection connection, const GT2Byte* message,
                               int len) {
  // send the data on the socket
  if (!gti2SocketSend(connection->socket, connection->ip, connection->port,
                      message, len))
    return GT2False;
...
static GT2Bool gti2SendRetries(GT2Connection connection, gsi_time now) {
  ...
  for (i = 0; i < len; i++) {
    // get the message
    message = (GTI2OutgoingBufferMessage*)ArrayNth(
        connection->outgoingBufferMessages, i);
