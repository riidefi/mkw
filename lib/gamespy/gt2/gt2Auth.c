...
static int gti2VerifyChallenge(const GT2Byte* buffer) {
  ...
  for (i = 1; i < GTI2_CHALLENGE_LEN; i++) {
    ...
    if ((oddmode && (buffer[i] & 1) == 0) ||
        (!oddmode && ((buffer[i] & 1) == 1)))
      return 0; // failed!!
  }
  return 1;
}
...
GT2Byte* gti2GetResponse(GT2Byte* buffer, const GT2Byte* challenge) {
  ...
  int chalrand;
  valid = gti2VerifyChallenge(
      challenge); // it's an invalid challenge, give them a bogus response
  // assert(GTI2_RESPONSE_LEN <= GTI2_CHALLENGE_LEN);
  for (i = 0; i < GTI2_RESPONSE_LEN; i++) {
    // use random vals for spots 0 and 13
    if (!valid || i == 0 || i == 13)
      buffer[i] = (GT2Byte)(33 + rand() % 93); // use chars in the range 33 -
                                               // 125
      ...
      chalrand = abs((challenge[((i * challenge[i]) +
                                 GT2ChallengeKey[(i + challenge[i]) % keylen]) %
                                GTI2_CHALLENGE_LEN] ^
