...
#define GS_SSL_PAD_ONE                                                         \
  "666666666666666666666666666666666666666666666666" // 48 bytes
...
typedef struct gsSSL {
  ...
  unsigned char
      sendSeqNBO[8]; // incrementing sequence number (for messages sent)
  ...
  unsigned char
      serverRandom[32]; // server random for key generation, sent plain text
  unsigned char
      clientRandom[32]; // client random for key generation, sent plain text
  unsigned char
      premastersecret[GS_SSL_MASTERSECRET_LEN]; // client random for key
                                                // generation, sent encrypted
                                                // with serverpub
...
typedef struct gsSSLClientHelloMsg {
  ...
  unsigned char sessionIdLen; // how many of the bytes that follow are session
                              // info? (def:0)
