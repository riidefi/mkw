...
//
// [in]		theUTF8String	:	UTF8String, doesn't need to be null
// terminated [out]	theUCS2Char		:	The 2 byte UCS2
// equivalent [in]     theMaxLength    :   Maximum number of *bytes* to read
// (not UTF8 characters)
//
// return value				:	The number of bytes read from
// theUTF8String
...
//
//	Security Concern:
//		Because data is routed through an ASCII stream prior to this
// function being 		called, embedded NULLs are stripped and hence,
// this function does not check for them 		For example, the UTF-8
// byte :1000 0000, would convert to a UCS2 NULL character 		If this
...
int _ReadUCS2CharFromUTF8String(const UTF8String theUTF8String,
  ...
  // Check for 2 byte UTF8
    ...
    if (UTF8_IS_FOLLOW_BYTE(theUTF8String[1])) {
      ...
      //		5 value bits from first UTF8Byte
      //(:000ABCDE) 		plus 6 value bits from the second UTF8Byte
      //(:00FGHIJK) 	Store as (:0000 0ABC DEFG HIJK)
      *theUnicodeChar =
          (unsigned short)(((theUTF8String[0] & UTF8_TWO_BYTE_MASK) << 6) +
  ...
  // Check for 3 byte UTF8
    ...
    if (UTF8_IS_FOLLOW_BYTE(theUTF8String[1]) &&
      ...
      //		4 value bits from first UTF8Byte
      //(:0000ABCD) 		plus 6 value bits from the second UTF8Byte
      //(:00EFGHIJ) 		plus 6 value bits from the third  UTF8Byte
      // (:00KLMNOP) 	Store as (:ABCD EFGH IJKL MNOP)
      *theUnicodeChar =
          (unsigned short)(((theUTF8String[0] & UTF8_THREE_BYTE_MASK) << 12) +
...
//
// return value				:	The length of theUTF8String in
// bytes
//
//  Remarks:
...
//		Memory allocated for theAsciiString may need to be as large as
// the UTF8String 		UTF8String will be NULL terminated
...
//
//	  Remarks:
//		Memory allocated for theUTF8String may need to be up to 1.5* the
// size of theUCS2String
//      This means that for each UCS2 character, 3 UTF8 characters may be
//      generated
int UCS2ToUTF8String(const UCS2String theUCS2String, UTF8String theUTF8String) {
  ...
  while (*anInStream != 0) {
    aUTF8CharLength = (unsigned int)_UCS2CharToUTF8String(
        *anInStream, (UTF8String)anOutStream);
...
int UTF8ToUCS2String(const UTF8String theUTF8String, UCS2String theUCS2String) {
  return UTF8ToUCS2StringLen(theUTF8String, theUCS2String,
                             (gsi_i32)strlen(theUTF8String));
}
...
//  [in]	theUTF8String, NULL terminated UTF8String
//
//  returns the length (in UCS2 characters) of theUCS2String that would be
//  created
//
//	  Remarks:
...
UCS2String UTF8ToUCS2StringAlloc(const UTF8String theUTF8String) {
  ...
  if (theUTF8String == NULL)
    ...
    UCS2String aUCS2String =
        (UCS2String)gsimalloc(sizeof(UCS2Char) * (newLength + 1));
...
UTF8String UCS2ToUTF8StringAlloc(const UCS2String theUCS2String) {
  ...
  if (theUCS2String == NULL)
    ...
    UTF8String aUTF8String =
        (UTF8String)gsimalloc(sizeof(char) * (newLength + 1));
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
// Convert a UTF8StringArray to a UCS2StringArray, allocate space for the
// UCS2Strings
//
...
//
//	  Remarks:
//		The callee is responsible for freeing the allocated memory
// block(s)
UCS2String* UTF8ToUCS2StringArrayAlloc(const UTF8String* theUTF8StringArray,
                                       int theNumStrings) {
  // Allow for NULL here since SDKs allow for NULL string arrays
  if (theUTF8StringArray == NULL || theNumStrings == 0)
    ...
    UCS2String* aUCS2StringArray =
        (UCS2String*)gsimalloc(sizeof(UCS2String) * theNumStrings);
    int stringNum = 0;
    while (stringNum < theNumStrings) {
      aUCS2StringArray[stringNum] =
          UTF8ToUCS2StringAlloc(theUTF8StringArray[stringNum]);
      stringNum++;
    }
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
// Convert a UCS2StringArray to a UTF8StringArray, allocate space for the
// UTF8Strings
//
...
//	  Remarks:
//		The callee is responsible for freeing the allocated memory block
UTF8String* UCS2ToUTF8StringArrayAlloc(const UCS2String* theUCS2StringArray,
                                       int theNumStrings) {
  // Allow for NULL here since SDKs allow for NULL string arrays
  if (theUCS2StringArray == NULL || theNumStrings == 0)
    ...
    UTF8String* aUTF8StringArray =
        (UTF8String*)gsimalloc(sizeof(UTF8String) * theNumStrings);
    int stringNum = 0;
    while (stringNum < theNumStrings) {
      aUTF8StringArray[stringNum] =
          UCS2ToUTF8StringAlloc(theUCS2StringArray[stringNum]);
      stringNum++;
    }
...
//
//	  Remarks:
//		The callee is responsible for allocating memory for
// theUCS2String 		the size returned should always be 2x the size
// passed in
int AsciiToUCS2String(const char* theAsciiString, UCS2String theUCS2String) {
  ...
  while (*aReadPos != '\0') {
    (*aWritePos++) =
        (unsigned short)(0x00FF & (*aReadPos++)); // copy and strip extra byte
    length++;
  }
...
//
//	  Remarks:
//		The length of theUTF8String will not exceed theMaxLength
supplied. int UCS2ToUTF8StringLength(const UCS2String theUCS2String, UTF8String
theUTF8String, int theMaxLength)
{
        return 0;
}
*/
...
//
//	  Remarks:
//		The length of theUCS2String will not exceed theMaxLength
// supplied.
int UTF8ToUCS2StringLen(const UTF8String theUTF8String,
                        UCS2String theUCS2String, int theMaxLength) {
  ...
  while (*anInStream != '\0' && theMaxLength > aTotalBytesRead) {
    ...
    aNumBytesRead = _ReadUCS2CharFromUTF8String(
        (UTF8String)anInStream, anOutStream, theMaxLength - aTotalBytesRead);
    if (aNumBytesRead == 0) {
