...
void ghiCallCompletedCallback(GHIConnection* connection) {
  ...
  if (connection->result != GHTTPSuccess) {
    gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Network,
                  GSIDebugLevel_WarmError, "Socket Error: %d\n",
  ...
  freeBuffer = connection->completedCallback(
      connection->request, connection->result, buffer, bufferLen,
...
void ghiCallProgressCallback(GHIConnection* connection, const char* buffer,
  ...
  // Call the callback.
  /////////////////////
  connection->progressCallback(connection->request, connection->state, buffer,
                               bufferLen, connection->fileBytesReceived,
...
void ghiCallPostCallback(GHIConnection* connection) {
  ...
  connection->postingState.callback(
      connection->request, connection->postingState.bytesPosted,
